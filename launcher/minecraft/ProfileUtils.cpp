...
namespace ProfileUtils
{
		...
		try
		{
			...
			if (version != currentOrderFileVersion)
			{
				throw JSONValidationError(
					QObject::tr("Invalid order file version, expected %1").arg(currentOrderFileVersion));
			}
	...
	VersionFilePtr parseJsonFile(const QFileInfo& fileInfo, const bool requireOrder)
	{
		...
		{
			auto errorStr =
				QObject::tr("Unable to open the version file %1: %2.").arg(fileInfo.fileName(), file.errorString());
