															...
															const ModPlatform::ResourceProvider providerName)
{
	if (auto isQuilt = m_loaderType & ModPlatform::Quilt; isQuilt || m_loaderType & ModPlatform::Fabric)
	{
		...
		auto over =
			std::find_if(overide.cbegin(),
						 overide.cend(),
						 [dep, providerName, isQuilt](const auto& o)
...
QList<ModPlatform::Dependency> GetModDependenciesTask::getDependenciesForVersion(
	...
	const ModPlatform::ResourceProvider providerName)
{
	...
	for (auto ver_dep : version.dependencies)
	{
		...
		auto isOnlyVersion =
			providerName == ModPlatform::ResourceProvider::MODRINTH && ver_dep.addonId.toString().isEmpty();
		if (auto dep =
				std::find_if(c_dependencies.begin(),
							 c_dependencies.end(),
		...
		if (auto dep = std::find_if(m_selected.begin(),
									...
									[&ver_dep, providerName, isOnlyVersion](std::shared_ptr<PackDependency> i)
									{
										return i->pack->provider == providerName
											&& (isOnlyVersion ? i->version.version == ver_dep.version
		...
		if (auto dep = std::find_if(m_mods.begin(),
									...
									[&ver_dep, providerName, isOnlyVersion](std::shared_ptr<Metadata::ModStruct> i)
									{
										return i->provider == providerName
											&& (isOnlyVersion ? i->file_id == ver_dep.version
															  : i->project_id == ver_dep.addonId);
									});
			dep != m_mods.end())
		...
		if (auto dep = std::find_if(m_pack_dependencies.begin(),
									...
									{
										return i->pack->provider == providerName
											&& (isOnlyVersion ? i->version.version == ver_dep.addonId
															  : i->pack->addonId == ver_dep.addonId);
									});
...
Task::Ptr GetModDependenciesTask::getProjectInfoTask(std::shared_ptr<PackDependency> pDep)
{
	...
	connect(info.get(),
			&NetJob::succeeded,
			[this, responseInfo, provider, pDep]
			{
				...
				try
				{
					auto obj = provider == ModPlatform::ResourceProvider::FLAME
								 ? Json::requireObject(Json::requireObject(doc), "data")
	...
	return pDep->version.fileName.isEmpty() ||
		...
		|| // check the selected versions
		   ...
		   std::find_if(m_mods_file_names.begin(),
			   ...
			   != m_mods_file_names.end()
		|| // check the existing mods

		   std::find_if(m_pack_dependencies.begin(),
						m_pack_dependencies.end(),
						[pDep](std::shared_ptr<PackDependency> i)
						{
...
{
	return std::find_if(m_mods_file_names.begin(),
						m_mods_file_names.end(),
						[pDep](QString i) { return !i.isEmpty() && laxCompare(i, pDep->version.fileName, true); })
