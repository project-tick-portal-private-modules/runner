...
namespace ShortcutUtils
{
	...
	bool createInstanceShortcut(const Shortcut& shortcut, const QString& filePath)
	{
		...
		auto icon =
			APPLICATION->icons()->icon(shortcut.iconKey.isEmpty() ? shortcut.instance->iconKey() : shortcut.iconKey);
		if (icon == nullptr)
		{
		...
		if (appPath.startsWith("/private/var/"))
		{
			QMessageBox::critical(
				...
				QObject::tr(
					"The launcher is in the folder it was extracted from, therefore it cannot create shortcuts."));
			return false;
		}
		...
		if (!iconFile.open(QFile::WriteOnly))
		{
			QMessageBox::critical(shortcut.parent,
								  QObject::tr("Create Shortcut"),
								  QObject::tr("Failed to create icon for application."));
			return false;
		}
		...
		if (appPath.startsWith("/tmp/.mount_"))
		{
			...
			if (appPath.isEmpty())
			{
				QMessageBox::critical(
					...
					QObject::tr("Create Shortcut"),
					QObject::tr(
						"Launcher is running as misconfigured AppImage? ($APPIMAGE environment variable is missing)"));
			}
		...
		if (!iconFile.open(QFile::WriteOnly))
		{
			QMessageBox::critical(shortcut.parent,
								  QObject::tr("Create Shortcut"),
								  QObject::tr("Failed to create icon for shortcut."));
			return false;
		}
		...
		QMessageBox::critical(shortcut.parent,
							  QObject::tr("Create Shortcut"),
							  QObject::tr("Not supported on your platform!"));
		return false;
#endif
	...
	bool createInstanceShortcutOnDesktop(const Shortcut& shortcut)
	{
		...
		if (desktopDir.isEmpty())
		{
			QMessageBox::critical(shortcut.parent,
								  QObject::tr("Create Shortcut"),
								  QObject::tr("Couldn't find desktop?!"));
			return false;
		}
	...
	bool createInstanceShortcutInApplications(const Shortcut& shortcut)
	{
		...
		if (applicationsDir.isEmpty())
		{
			QMessageBox::critical(shortcut.parent,
								  QObject::tr("Create Shortcut"),
								  QObject::tr("Couldn't find applications folder?!"));
			return false;
		}
	...
	bool createInstanceShortcutInOther(const Shortcut& shortcut)
	{
		...
		QString shortcutFilePath =
			FS::PathCombine(defaultedDir, FS::RemoveInvalidFilenameChars(shortcut.name) + extension);
		QFileDialog fileDialog;
		// workaround to make sure the portal file dialog opens in the desktop directory
		fileDialog.setDirectoryUrl(defaultedDir);

		shortcutFilePath = fileDialog.getSaveFileName(shortcut.parent,
													  QObject::tr("Create Shortcut"),
													  shortcutFilePath,
