...
auto PasteUpload::Sink::finalize(QNetworkReply& reply) -> Task::State
{
	...
	else if (statusCode != 200 && statusCode != 201)
	{
		...
		m_fail_reason		 = QObject::tr("Error: %1 returned unexpected status code %2 %3")
							.arg(m_d->url().toString())
							.arg(statusCode)
							.arg(reasonPhrase);
		return Task::State::Failed;
	}
	...
	{
		case PasteUpload::NullPointer: m_d->m_pasteLink = QString::fromUtf8(*m_output).trimmed(); break;
		case PasteUpload::Hastebin:
		{
			...
			if (jsonError.error != QJsonParseError::NoError)
			{
				...
				m_fail_reason = QObject::tr("Failed to parse response from hastebin server: expected JSON but got an "
		...
		case PasteUpload::Mclogs:
		{
			...
			if (jsonError.error != QJsonParseError::NoError)
			{
				...
				m_fail_reason = QObject::tr("Failed to parse response from mclogs server: expected JSON but got an "
											"invalid response. Error: %1")
									.arg(jsonError.errorString());
				return Task::State::Failed;
			}
		...
		case PasteUpload::PasteGG:
			...
			if (jsonError.error != QJsonParseError::NoError)
			{
				...
				m_fail_reason = QObject::tr("Failed to parse response from pasteGG server: expected JSON but got an "
											"invalid response. Error: %1")
									.arg(jsonError.errorString());
				return Task::State::Failed;
			}
			...
			if (obj.contains("status") && obj["status"].isString())
			{
				...
				else
				{
					QString error = obj["error"].toString();
					QString message =
						(obj.contains("message") && obj["message"].isString()) ? obj["message"].toString() : "none";
...
PasteUpload::PasteUpload(const QString& log, QString url, PasteType pasteType)
	: m_log(log),
	  m_baseUrl(url),
	  m_paste_type(pasteType)
{
	...
	// HACK: Paste's docs say the standard API path is at /api/<version> but the official instance paste.gg doesn't
	// follow that??
