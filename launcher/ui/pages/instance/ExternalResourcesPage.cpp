...
ExternalResourcesPage::ExternalResourcesPage(BaseInstance* instance,
	...
	: QMainWindow(parent),
	  m_instance(instance),
	  ui(new Ui::ExternalResourcesPage),
	  m_model(model)
{
	ui->setupUi(this);
...
bool ExternalResourcesPage::listFilter(QKeyEvent* keyEvent)
{
	switch (keyEvent->key())
	{
		case Qt::Key_Delete: removeItem(); return true;
		case Qt::Key_Plus: addItem(); return true;
		default: break;
	}
...
void ExternalResourcesPage::updateActions()
{
	...
	const QModelIndexList selection =
		m_filterModel->mapSelectionToSource(ui->treeView->selectionModel()->selection()).indexes();
	...
	ui->actionViewHomepage->setEnabled(hasSelection
									   && std::any_of(selectedResources.begin(),
													  selectedResources.end(),
													  [](Resource* resource)
													  { return !resource->homepage().isEmpty(); }));
...
QString ExternalResourcesPage::extraHeaderInfoString()
{
	...
	{
		auto selection = m_filterModel->mapSelectionToSource(ui->treeView->selectionModel()->selection()).indexes();
		if (auto count = std::count_if(selection.cbegin(), selection.cend(), [](auto v) { return v.column() == 0; });
			count != 0)
