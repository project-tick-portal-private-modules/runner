...
namespace Flame
{
	...
	void ListModel::getLogo(const QString& logo, const QString& logoUrl, LogoCallback callback)
	{
		...
		{
			callback(
				APPLICATION->metacache()->resolveEntry("FlamePacks", QString("logos/%1").arg(logo))->getFullPath());
		}
		else
		{
	...
	void ListModel::searchWithTerm(const QString& term,
								   ...
								   bool filterChanged)
	{
		if (m_currentSearchTerm == term && m_currentSearchTerm.isNull() == term.isNull() && m_currentSort == sort
			&& !filterChanged)
		{
			return;
		}
