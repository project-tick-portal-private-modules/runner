...
namespace ResourceDownload
{
	...
	FlameModPage::FlameModPage(ModDownloadDialog* dialog, BaseInstance& instance) : ModPage(dialog, instance)
	{
		...
		connect(m_ui->packView->selectionModel(),
				...
				this,
				&FlameModPage::onSelectionChanged);
		...
		: ResourcePackResourcePage(dialog, instance)
	{
		...
		connect(m_ui->packView->selectionModel(),
				&QItemSelectionModel::currentChanged,
				this,
				&FlameResourcePackPage::onSelectionChanged);
		connect(m_ui->versionSelectionBox,
				&QComboBox::currentIndexChanged,
				this,
	...
	void FlameResourcePackPage::openUrl(const QUrl& url)
	{
		if (url.scheme().isEmpty())
		{
			...
			if (query.startsWith("remoteUrl="))
			{
				// attempt to resolve url from warning page
				query.remove(0, 10);
				ResourcePackResourcePage::openUrl({ QUrl::fromPercentEncoding(query.toUtf8()) }); // double decoding is
																								  // necessary
				return;
			}
		}
		...
		: TexturePackResourcePage(dialog, instance)
	{
		...
		connect(m_ui->packView->selectionModel(),
				&QItemSelectionModel::currentChanged,
				this,
				&FlameTexturePackPage::onSelectionChanged);
		connect(m_ui->versionSelectionBox,
				&QComboBox::currentIndexChanged,
				this,
	...
	void FlameDataPackPage::openUrl(const QUrl& url)
	{
		if (url.scheme().isEmpty())
		{
			...
			if (query.startsWith("remoteUrl="))
			{
				// attempt to resolve url from warning page
				query.remove(0, 10);
				DataPackResourcePage::openUrl({ QUrl::fromPercentEncoding(query.toUtf8()) }); // double decoding is
																							  // necessary
				return;
			}
		}
		...
		: ShaderPackResourcePage(dialog, instance)
	{
		...
		connect(m_ui->packView->selectionModel(),
				&QItemSelectionModel::currentChanged,
				this,
				&FlameShaderPackPage::onSelectionChanged);
		connect(m_ui->versionSelectionBox,
				&QComboBox::currentIndexChanged,
				this,
		...
		: DataPackResourcePage(dialog, instance)
	{
		...
		connect(m_ui->packView->selectionModel(),
				&QItemSelectionModel::currentChanged,
				this,
				&FlameDataPackPage::onSelectionChanged);
		connect(m_ui->versionSelectionBox,
				&QComboBox::currentIndexChanged,
				this,
	...
	void FlameShaderPackPage::openUrl(const QUrl& url)
	{
		if (url.scheme().isEmpty())
		{
			...
			if (query.startsWith("remoteUrl="))
			{
				// attempt to resolve url from warning page
				query.remove(0, 10);
				ShaderPackResourcePage::openUrl({ QUrl::fromPercentEncoding(query.toUtf8()) }); // double decoding is
																								// necessary
				return;
			}
		}
