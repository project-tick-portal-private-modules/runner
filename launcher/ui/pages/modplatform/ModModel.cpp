...
namespace ResourceDownload
{
	...
	bool ModModel::isPackInstalled(ModPlatform::IndexedPack::Ptr pack) const
	{
		...
		return std::any_of(allMods.cbegin(),
						   allMods.cend(),
						   [pack](Mod* mod)
						   {
	...
	QVariant ModModel::getInstalledPackVersion(ModPlatform::IndexedPack::Ptr pack) const
	{
		...
		for (auto mod : allMods)
		{
			if (auto meta = mod->metadata();
				meta && meta->provider == pack->provider && meta->project_id == pack->addonId)
			{
				return meta->version();
			}
		}
	...
	{
		return filter == ModPlatform::Side::NoSide || value == ModPlatform::Side::NoSide
			|| filter == ModPlatform::Side::UniversalSide || value == ModPlatform::Side::UniversalSide
			|| filter == value;
	}
	...
	bool ModModel::checkVersionFilters(const ModPlatform::IndexedVersion& v)
	{
		...
		return (!optedOut(v) && // is opted out(aka curseforge download link)
				...
				(m_filter->releases.empty() ||										   // releases
				 std::find(m_filter->releases.cbegin(), m_filter->releases.cend(), v.version_type)
					 != m_filter->releases.cend())
