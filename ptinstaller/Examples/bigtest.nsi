; bigtest.nsi
;
; This script attempts to test most of the functionality of the PTIS exehead.
...
InstallDir "$PROGRAMFILES\PTISTest\BigPTISTest"
InstallDirRegKey HKLM "Software\PTISTest\BigPTISTest" "Install_Dir"

CheckBitmap "${PTISDIR}\Contrib\Graphics\Checks\classic-cross.bmp"
...
Section "" ; empty string makes it hidden, so would starting with -
  ...
  WriteRegStr HKLM SOFTWARE\PTISTest\BigPTISTest "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\BigPTISTest" "DisplayName" "BigPTISTest (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\BigPTISTest" "UninstallString" '"$INSTDIR\bt-uninst.exe"'
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\PTISTest\BigPTISTest "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\PTISTest\BigPTISTest "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\PTISTest\BigPTISTest "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\PTISTest\BigPTISTest "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\PTISTest\BigPTISTest "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
...
Function "CSCTest"
  ...
  CreateDirectory "$SMPROGRAMS\Big PTIS Test"
  SetOutPath $INSTDIR ; for working directory
  CreateShortcut "$SMPROGRAMS\Big PTIS Test\Uninstall BIG PTIS Test.lnk" "$INSTDIR\bt-uninst.exe" ; use defaults for parameters, icon, etc.
  ; this one will use notepad's icon, start it minimized, and give it a hotkey (of Ctrl+Shift+Q)
  CreateShortcut "$SMPROGRAMS\Big PTIS Test\silent.nsi.lnk" "$INSTDIR\silent.nsi" "" "$WINDIR\notepad.exe" 0 SW_SHOWMINIMIZED CONTROL|SHIFT|Q
  CreateShortcut "$SMPROGRAMS\Big PTIS Test\TheDir.lnk" "$INSTDIR\" "" "" 0 SW_SHOWMAXIMIZED CONTROL|SHIFT|Z
...
UninstallIcon "${PTISDIR}\Contrib\Graphics\Icons\ptis1-uninstall.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\BigPTISTest"
  DeleteRegKey HKLM "SOFTWARE\PTISTest\BigPTISTest"
  Delete "$INSTDIR\silent.nsi"
  Delete "$INSTDIR\LogicLib.nsi"
  Delete "$INSTDIR\bt-uninst.exe"
  Delete "$INSTDIR\test.ini"
  Delete "$SMPROGRAMS\Big PTIS Test\*.*"
  RMDir "$SMPROGRAMS\BiG PTIS Test"
