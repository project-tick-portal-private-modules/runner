...
SVNROOT=https://svn.code.sf.net/p/ptis/code/PTIS
...
[rsh]
RSH="C:\Program Files\PuTTY\plink.exe" -2 -l kichik ptis.sourceforge.net

[sftp]
SFTP="C:\Program Files\PuTTY\psftp.exe" -2 -l kichik,ptis -batch -b %s frs.sourceforge.net

[wiki]
PURGE_URL=https://ptis.sourceforge.io/%s?action=purge
UPDATE_URL=https://ptis.sourceforge.io/Special:Simpleupdate?action=raw
...
def TestInstaller():
	...
	run(
		'..\\ptis-test-setup.exe /S /D=%s\\insttest' % os.getcwd(),
		LOG_NOTHING,
		'installer failed'
	)

	run(
		'diff -r insttest insttestscons | grep -v uninst-ptis.exe | grep -v PTIS.exe',
		LOG_ALL,
...
def CreateSpecialBuilds():
	...
	create_special_build('strlen_8192', 'PTIS_MAX_STRLEN=8192')
	create_special_build('log', 'PTIS_CONFIG_LOG=yes')

def UploadFiles():
	print('uploading files to SourceForge...')

	folder = 'PTIS 3/' + VERSION
	if PRE_RELEASE_VERSION:
		folder = 'PTIS 3 Pre-release/' + VERSION

	sftpcmds = open('sftp-commands', 'w')
	sftpcmds.write('mkdir "/home/frs/project/ptis/%s"\n' % folder)
	sftpcmds.write('cd "/home/frs/project/ptis/%s"\n' % folder)
	sftpcmds.write('put %s.tar.bz2\n' % newverdir)
	sftpcmds.write('put %s\\ptis-%s-setup.exe\n' % (newverdir, VERSION))
	sftpcmds.write('put %s\\ptis-%s.zip\n' % (newverdir, VERSION))
	sftpcmds.write('put ptis-%s-log.zip\n' % VERSION)
	sftpcmds.write('put ptis-%s-strlen_8192.zip\n' % VERSION)
...
def UpdateWiki():
	...
	if not PRE_RELEASE_VERSION:
		update_wiki_page('Template:PTISVersion', VERSION, 'new version')
		update_wiki_page('Template:PTISReleaseDate', time.strftime('%B %d, %Y'), 'new version')
		update_wiki_page('Template:PTISIsPreRelease', 'no', 'new version')
	else:
		update_wiki_page('Template:PTISPreVersion', VERSION, 'new version')
		update_wiki_page('Template:PTISPreReleaseDate', time.strftime('%B %d, %Y'), 'new version')
		update_wiki_page('Template:PTISIsPreRelease', 'yes', 'new version')
