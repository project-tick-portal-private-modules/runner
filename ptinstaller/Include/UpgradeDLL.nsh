...
!macro __UpgradeDLL_Helper_AddRegToolEntry mode filename tempdir
  ...
  StrCpy $R0 0
  ReadRegDWORD $R0 HKLM "Software\PTIS.Library.RegTool.v3\UpgradeDLLSession" "count"
  IntOp $R0 $R0 + 1
  WriteRegDWORD HKLM "Software\PTIS.Library.RegTool.v3\UpgradeDLLSession" "count" "$R0"

  ;------------------------
  ;Setup RegTool

  !if ! /FileExists "${PTISDIR}\Bin\RegTool-${PTIS_CPU}.bin"
    !error "Missing RegTool for ${PTIS_CPU}!"
  !endif

  ReadRegStr $R3 HKLM "Software\Microsoft\Windows\CurrentVersion\RunOnce" "PTIS.Library.RegTool.v3"
  StrCpy $R3 $R3 -4 1
  IfFileExists $R3 +3

    File /oname=$R2\PTIS.Library.RegTool.v3.$HWNDPARENT.exe "${PTISDIR}\Bin\RegTool-${PTIS_CPU}.bin"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\RunOnce" \
      "PTIS.Library.RegTool.v3" '"$R2\PTIS.Library.RegTool.v3.$HWNDPARENT.exe" /S'

  ;------------------------
  ;Add RegTool entry

  WriteRegStr HKLM "Software\PTIS.Library.RegTool.v3\UpgradeDLLSession" "$R0.file" "$R1"
  WriteRegStr HKLM "Software\PTIS.Library.RegTool.v3\UpgradeDLLSession" "$R0.mode" "${mode}"
