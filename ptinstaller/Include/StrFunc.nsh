...
(-----------------------------------------------------------------------------)
| By deguix                                     / A Header file for PTIS 2.01 |
| <cevo_deguix@yahoo.com.br>                   -------------------------------|
|                                                                             |
|    This header file contains PTIS functions for string manipulation.        |
|                                                                    ---------|
...
!ifndef STRFUNC
 ...
 ;!define STRFUNC_VERBLD 0
  ...
  !echo `${STRFUNC_INITMSGPRE}PTIS ${STRFUNC} ${STRFUNC_VER} - Copyright ${STRFUNC_CREDITS}${STRFUNC_INITMSGPOST}`
  !verbose pop
  ...
  ############################################################################  
  # StrIOToPTIS
  !insertmacro STRFUNC_DEFFUNC StrIOToPTIS `ResultVar|String` `Output|Text`

  !macro STRFUNC_CALL_StrIOToPTIS un ResultVar String
    !verbose push ${STRFUNC_VERBOSITY}
    !echo `${U+24}{${un}StrIOToPTIS} "${ResultVar}" "${String}"`
    !verbose 2
    Push `${String}`
    !insertmacro STRFUNC_CALL StrIOToPTIS "${un}"
    Pop ${ResultVar}
    !verbose pop
  !macroend

  !macro STRFUNC_MAKEFUNC_StrIOToPTIS
  ...
  ############################################################################  
  # StrPTISToIO
  !insertmacro STRFUNC_DEFFUNC StrPTISToIO `ResultVar|String` `Output|Text`

  !macro STRFUNC_CALL_StrPTISToIO un ResultVar String
    !verbose push ${STRFUNC_VERBOSITY}
    !echo `${U+24}{${un}StrPTISToIO} "${ResultVar}" "${String}"`
    !verbose 2
    Push `${String}`
    !insertmacro STRFUNC_CALL StrPTISToIO "${un}"
    Pop ${ResultVar}
    !verbose pop
  !macroend

  !macro STRFUNC_MAKEFUNC_StrPTISToIO
    !insertmacro STRFUNC_BEGINFUNC ${basename} `${un}` `2004 "bluenet" - Based on functions by Amir Szekely, Joost Verburg, Dave Laundon and Diego Pedroso`
