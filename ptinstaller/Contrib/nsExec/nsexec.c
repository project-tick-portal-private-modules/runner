...
void ExecScript(int mode) {
  ...
  *szRet = _T('\0');
  ...
  // g_hwndParent = the caller, usually PTIS installer.
...
params:
  // Get the command I need to run from the PTIS stack.
  popstring(pExec);
...
resizeOutputBuffer:
            if (stackMode) {
              ignoreData = TRUE; // Buffer was already maximum for the PTIS stack, we cannot handle more data
              if (utfOutput) 
