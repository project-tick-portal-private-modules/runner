...
#include <windows.h>
#include <ptis/pluginapi.h> // ptis plugin
#include "MyMath.h"
#include "Math.h"
...
#define PTIS_VARS_COUNT 27
#define PTIS_VARS_STACK 25
#define PTIS_VARS_NSTACK 26

typedef TCHAR smallstr[2];
const smallstr PTISVariablesNames[PTIS_VARS_COUNT] = {{_T('r'),_T('0')}, {_T('r'),_T('1')}, {_T('r'),_T('2')}, {_T('r'),_T('3')}, {_T('r'),_T('4')}, {_T('r'),_T('5')}, {_T('r'),_T('6')}, {_T('r'),_T('7')}, {_T('r'),_T('8')}, {_T('r'),_T('9')},
...
ExpressionItem *FindVariable(TCHAR *varname)
{
    ...
    // check PTIS variables
    for (i = 0; i < PTIS_VARS_COUNT; i++)
    {
        if (lstrcmpn(varname, PTISVariablesNames[i],2) == 0)
        {
            if (i == PTIS_VARS_STACK) item->type = IT_VARIABLE | ITV_STACK;
            else if (i == PTIS_VARS_NSTACK) item->type = IT_VARIABLE | ITV_NSTACK;
            else
                item->type = (IT_VARIABLE | ITV_PTIS) + i;
            return item;
        }
    }
...
void ParseString(TCHAR *&sp, ExpressionItem* &itemplace)
{
        ...
        (*sp != _T(']')) && (*sp != _T(',')))
    {
        ...
        case _T('{'):   // start of array define
            {
                // array define - coptists of array copy operator and array define itself
        ...
        *place = 0;
        ...
        case IT_VARIABLE:
            ...
            case ITV_PTIS:
                {
                TCHAR buffer[128];
                buffer[0] = PTISVariablesNames[item->type & ITEMOPTIONS][0];
                buffer[1] = PTISVariablesNames[item->type & ITEMOPTIONS][1];
                buffer[2] = 0;
...
void SaveResult(ExpressionItem *var, ExpressionItem *result)
{
    ...
    case ITV_PTIS:
        {
            // store string result direct to PTIS variable
...
void RunTree(ExpressionItem *from, ExpressionItem* &result, int options)
{
    ...
    while (item != NULL)
    {
        ...
        case IT_VARIABLE:
            ...
            {
                // we need const result - :) is it ptis or common variable
                switch (subtype)
                {
                case ITV_PTIS:
                    {
