...
template<class T> int RegTool()
{
  ...
  if (*cmdline == ('S'))
  {
    ...
    if (SUCCEEDED(RegOpenKeyExA(HKEY_LOCAL_MACHINE, "Software\\PTIS.Library.RegTool.v3", 0, KEY_READ, &rootkey)))
    {
      DWORD keyidx = 0;
      ...
      RegCloseKey(rootkey);
      RegDeleteKeyA(HKEY_LOCAL_MACHINE, "Software\\PTIS.Library.RegTool.v3");
    }
