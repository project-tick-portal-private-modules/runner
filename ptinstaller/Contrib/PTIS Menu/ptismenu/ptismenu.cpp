...
#endif
    if ((!CanOpenChm() || !wxFileExists(path)) && 0 == location.CmpNoCase(wxT("PTIS.chm")))
    {
...
// simple menu events like this the static method is much simpler.
   ...
   bool MyApp::OnInit()
   {
     ...
     MyFrame *frame = new MyFrame(_("PTIS Menu"),
...
// main frame
// ----------------------------------------------------------------------------
#ifdef PTISMENU_NOLINKTAGHANDLER
...
#ifdef __WXMSW__
      this->SetIcon(wxICON(ptisicon));
#else
      wxIcon icon(wxT("ptisicon.ico"), wxBITMAP_TYPE_ICO);
      this->SetIcon(icon);
#endif
