...
{$IFDEF UNICODE}
type PTISTString = System.WideString; // UnicodeString?
type PTISPTChar = PWideChar;
{$ELSE}
type PTISTString = AnsiString;
type PTISPTChar = PAnsiChar;
{$ENDIF}
...
type
  ...
  Tvalidate_filename = procedure (const filename: PTISPTChar); stdcall;
...
var
  ...
  g_stacktop: ^pstack_t;
  g_variables: PTISPTChar;
  g_hwndParent: HWND;
  g_hwndList: HWND;
  g_hwndLogList: HWND;
  g_extraparameters: pextrap_t;

procedure Init(const hwndParent: HWND; const string_size: integer; const variables: PTISPTChar; const stacktop: pointer; const extraparameters: pointer = nil);
function LogMessage(Msg : String): BOOL;
function Call(PTIS_func : String) : Integer;
function PopString(): PTISTString;
procedure PushString(const str: PTISTString='');
function GetUserVariable(const varnum: TVariableList): PTISTString;
procedure SetUserVariable(const varnum: TVariableList; const value: PTISTString);
procedure PTISDialog(const text, caption: PTISTString; const buttons: integer);
...
function Call(PTIS_func : String) : Integer;
var
  codeoffset: Integer; //The ID of ptis function
begin
  Result := 0;
  codeoffset := StrToIntDef(PTIS_func, 0);
