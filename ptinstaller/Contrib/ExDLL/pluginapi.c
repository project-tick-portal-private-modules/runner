...
#define isvalidptisvarindex(varnum) ( ((unsigned int)(varnum)) < (__INST_LAST) )
...
int PTISCALL popstring(LPTSTR str)
{
  stack_t *th;
...
int PTISCALL popstringn(LPTSTR str, int maxlen)
{
  stack_t *th;
  if (!g_stacktop || !*g_stacktop) return 1;
  th=(*g_stacktop);
...
{
  if (!isvalidptisvarindex(varnum)) return NULL;
  return g_variables+varnum*g_stringsize;
}

void PTISCALL setuservariable(const int varnum, LPCTSTR var)
{
  if (var && isvalidptisvarindex(varnum)) 
...
int PTISCALL PopStringNA(LPSTR ansiStr, int maxlen)
{
...
{
   if (ansiStr && isvalidptisvarindex(varnum))
   {
...
int PTISCALL PopStringNW(LPWSTR wideStr, int maxlen)
{
   int realLen = maxlen ? maxlen : g_stringsize;
...
{
   if (wideStr && isvalidptisvarindex(varnum))
   {
...
unsigned int PTISCALL myatou(LPCTSTR s)
{
  unsigned int v=0;
...
int PTISCALL myatoi_or(LPCTSTR s)
{
  int v=0;
  if (*s == _T('0') && (s[1] == _T('x') || s[1] == _T('X')))
  {
...
INT_PTR PTISCALL popintptr()
{
  ...
  if (popstringn(buf,COUNTOF(buf)))
    return 0;
  return ptishelper_str_to_ptr(buf);
}

int PTISCALL popint_or()
{
  TCHAR buf[128];
  if (popstringn(buf,COUNTOF(buf)))
    return 0;
