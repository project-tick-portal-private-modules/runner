PTISdl 1.3 - HTTP downloading plugin for PTIS
---------------------------------------------
...
PTISdl::download http://www.domain.com/file localfile.exe

You can also pass /TIMEOUT to set the timeout in milliseconds:

PTISdl::download /TIMEOUT=30000 http://www.domain.com/file localfile.exe
...
If you don't want the progress window to appear, use PTISdl::download_quiet.
...
PTISdl supports only basic configurations of proxies. It doesn't support
proxies which require authentication, automatic configuration script, etc.
PTISdl reads the proxy configuration from Internet Explorer's registry key
under HKLM\Software\Microsoft\Windows\CurrentVersion\Internet Settings. It
reads and parses ProxyEnable and ProxyServer.

If you don't want PTISdl to use Internet Explorer's settings, use the
/NOIEPROXY flag. /NOIEPROXY should be used after /TRANSLATE and
/TIMEOUT. For example:

If you want to specify a proxy on your own, use the /PROXY flag.

PTISdl::download /NOIEPROXY http://www.domain.com/file localfile.exe
PTISdl::download /TIMEOUT=30000 /NOIEPROXY http://www.domain.com/file localfile.exe
PTISdl::download /PROXY proxy.whatever.com http://www.domain.com/file localfile.exe
PTISdl::download /PROXY proxy.whatever.com:8080 http://www.domain.com/file localfile.exe

TRANSLATE
---------

To translate PTISdl add the following values to the call line:
