...
void SetTitle(HWND hwnd,const TCHAR *substr) {
  TCHAR title[64];
  wsprintf(title,substr ? _T("MakePTISW - %s") : _T("MakePTISW"),substr);
...
#include <tom.h>
#define RE_HAS_TOM
#define IID_ITextDocument PTIS_IID_ITextDocument
...
void ClearLog() {
  ...
  SendMessage(g_sdata.hwnd, WM_MAKEPTIS_UPDATEUISTATE, 0, 0);
}

void LogMessage(HWND hwnd,const TCHAR *str) {
  HWND hLogWin = g_sdata.logwnd;
#ifdef RE_HAS_TOM
  HRESULT hr = (HRESULT) SendMessage(hwnd, WM_MAKEPTIS_FREEZEEDITOR, 0, true); // Force COM calls to UI thread
#endif
...
#ifdef RE_HAS_TOM
  if (SUCCEEDED(hr)) SendMessage(hwnd, WM_MAKEPTIS_FREEZEEDITOR, 0, false);
#endif
...
void EnableDisableItems(int on) 
{
  ...
  SendMessage(hwndDlg, WM_MAKEPTIS_UPDATEUISTATE, 0 ,0);
...
void CompilePTISScript() {
  ...
  PostMessage(g_sdata.hwnd, WM_MAKEPTIS_UPDATEUISTATE, 0, 0);
  if (!g_sdata.script[0]) {
  ...
  if (!g_sdata.compile_command) {
    ...
    size_t byteSize = sizeof(TCHAR)*(
      /* makeptis.exe        */ lstrlen(EXENAME)        + /* space */ 1 +
  ...
  DWORD tid;
  g_sdata.thread=CreateThread(NULL,0,MakePTISProc,0,0,&tid);
}
...
int InitBranding() {
  ...
  if (InitSpawn(si, read_stdout, newstdout)) {
    ...
    if (CreateProcess(0, cmd, 0, 0, TRUE, CREATE_NEW_CONSOLE, 0, 0, &si, &pi)) {
      ...
      wsprintf(g_sdata.branding, _T("PTIS %hs"), szBuf);
