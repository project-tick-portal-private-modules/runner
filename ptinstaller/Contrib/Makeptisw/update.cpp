...
{
  char buf[300];
  wsprintfA(buf, "%s%s", PTIS_UC_URL, g_sdata.brandingv);

  InternetAttemptConnect(0);
  UINT mbicon = MB_ICONINFORMATION;
  LPCSTR msg = NULL;
  HINTERNET hInet = InternetOpenA("MakePTISw (WinInet)", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
  ...
  if (hInet)
  {
    ...
    if (hConn)
    {
      ...
      if (!gle)
      {
        ...
        {
        case '0':
          msg = "There is no update available for PTIS at this time.";
          break;
        case '1':
        case '2':
          if (cbRecv > 2 && '|' == response[1])
          {
            ...
            wsprintfA(buf, "PTIS %.50s is now available. Would you like to download %s now?", response+2, reltypestr);
  ...
  if (IsWindow(hwndOwner))
  {
    ...
    {
      OpenUrlInDefaultBrowser(hwndOwner, PTIS_DL_URL);
    }

    EnableMenuItem(g_sdata.menu, IDM_PTISUPDATE, MF_ENABLED);
  }
  return 0;
}
...
void CheckForUpdate()
{
  ...
  EnableMenuItem(g_sdata.menu, IDM_PTISUPDATE, MF_GRAYED);
  DWORD tid;
