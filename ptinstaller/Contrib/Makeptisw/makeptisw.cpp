...
#ifdef _WIN64
  const TCHAR* SigintEventNameFmt = _T("makeptis win32 sigint event %Iu");
#else
  const TCHAR* SigintEventNameFmt = _T("makeptis win32 sigint event %u");
#endif
  const TCHAR* SigintEventNameLegacy = _T("makeptis win32 signint event");
}
...
int WINAPI _tWinMain(HINSTANCE hInst,HINSTANCE hOldInst,LPTSTR CmdLineParams,int ShowCmd) {
  ...
  g_sdata.sigint_event_legacy = CreateEvent(NULL, FALSE, FALSE, MakeptisAPI::SigintEventNameLegacy);
  ...
  if (!InitBranding()) {
    MessageBox(0,PTISERROR,ERRBOXTITLE,MB_ICONEXCLAMATION|MB_OK|MB_TASKMODAL);
    return 1;
  }
  ResetObjects();
...
INT_PTR CALLBACK DialogProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
  switch (msg) {
    ...
    case WM_MAKEPTIS_PROCESSCOMPLETE:
    {
      if (g_sdata.thread) {
      ...
      if(g_sdata.compressor == COMPRESSOR_BEST) {
        if (g_sdata.retcode==0 && FileExists(g_sdata.output_exe)) {
          ...
          wsprintf(temp_file_name,_T("%s_makeptisw_temp"),g_sdata.output_exe);
    ...
    case MakeptisAPI::QUERYHOST: {
      if (MakeptisAPI::QH_OUTPUTCHARSET == wParam) {
        const UINT reqcp = 1200; // We want UTF-16LE
        return DlgRet(hwndDlg, (LONG_PTR)(1+reqcp));
      }
      else if (MakeptisAPI::QH_SUPPORTEDVERSION == wParam)
    ...
    case WM_COPYDATA:
    {
      ...
      switch (cds->dwData) {
        case MakeptisAPI::NOTIFY_SCRIPT:
          ...
          break;
        case MakeptisAPI::NOTIFY_WARNING:
          g_sdata.warnings++;
          break;
        case MakeptisAPI::NOTIFY_ERROR:
          break;
        case MakeptisAPI::NOTIFY_OUTPUT:
          ...
          break;
        case MakeptisAPI::PROMPT_FILEPATH:
    ...
    case WM_INITMENU:
      ...
      break;
    case WM_MAKEPTIS_UPDATEUISTATE:
    {
    ...
    case WM_COMMAND:
    {
      switch (LOWORD(wParam)) {
        ...
        case IDM_PTISHOME: return OpenUrlInDefaultBrowser(g_sdata.hwnd, PTIS_URL);
        case IDM_FORUM: return OpenUrlInDefaultBrowser(g_sdata.hwnd, PTIS_FORUM_URL);
        case IDM_PTISUPDATE: return (CheckForUpdate(), TRUE);
        ...
        case IDM_LOADSCRIPT:
        {
          if (!g_sdata.thread) {
            ...
            TCHAR buf[MAX_PATH];
            l.hwndOwner = hwndDlg;
            l.lpstrFilter = _T("PTIS Script (*.nsi)\0*.nsi\0All Files (*.*)\0*.*\0");
            l.lpstrFile = buf;
        ...
        case IDM_GUIDGEN:
        {
          ...
          SendMessage(g_sdata.hwnd, WM_MAKEPTIS_UPDATEUISTATE, 0, 0); // Update clear log command state
          break;
        }
        case IDM_ARP:
        {
...
DWORD WINAPI MakePTISProc(LPVOID TreadParam) {
  TCHAR eventnamebuf[100];
  wsprintf(eventnamebuf, MakeptisAPI::SigintEventNameFmt, g_sdata.hwnd);
  if (g_sdata.sigint_event) CloseHandle(g_sdata.sigint_event);
  ...
  if (!g_sdata.sigint_event) {
    ...
    PostMessage(g_sdata.hwnd, WM_MAKEPTIS_PROCESSCOMPLETE, 0, 0);
    return 1;
  }
  ...
  switch(msg) {
    case WM_DRAWITEM:
      if (wParam == IDC_ABOUTHEADER)
      {
        ...
        RECT r, r2;
        const TCHAR txt[] = TEXT("MakePTISW");
    ...
    case WM_INITDIALOG:
    {
      ...
      static const BYTE fontnormctlids[] = { IDC_ABOUTCOPY, IDC_ABOUTPORTIONS, IDC_ABOUTDONATE, IDC_OTHERCONTRIB, IDC_PTISVER };
      ...
      SetDlgItemText(hwndDlg, IDC_PTISVER, g_sdata.branding);
    ...
    case WM_APP: showversion:
      {
        ...
        wsprintf(buf, _T("MakePTISW %s%s(PTIS Compiler Interface)"), showver ? PTISW_VERSION : _T(""), showver ? _T(" ") : _T(""));
...
INT_PTR CALLBACK SettingsProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
  switch(msg) {
    ...
    case WM_MAKEPTIS_LOADSYMBOLSET:
    {
    ...
    case WM_MAKEPTIS_SAVESYMBOLSET:
    {
      TCHAR *name = (TCHAR *)wParam;
      ...
      switch (LOWORD(wParam)) {
        case IDOK:
        {
          ...
          if (!*name) {
            ...
            UINT msg = dd.LoadingMode ? WM_MAKEPTIS_LOADSYMBOLSET : WM_MAKEPTIS_SAVESYMBOLSET;
