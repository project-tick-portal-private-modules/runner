...
DWORD WINAPI ThreadProc(LPVOID p) // thread that will start & monitor makeptis
{
  TCHAR buf[1024];
  ...
  */
      ...
      NULL,tempzip_path,&si,&pi))
  {
    ...
    wnd_printf(_T("\r\nPlease make sure the path to makeptis.exe is correct."));
...
void makeEXE(HWND hwndDlg)
{
  ...
  _ftprintf(fp,_T("!include `${PTISDIR}\\Contrib\\zip2exe\\Base.nsh`\n"));
  _ftprintf(fp,_T("!include `${PTISDIR}\\Contrib\\zip2exe\\%s.nsh`\n"),g_mui?_T("Modern"):_T("Classic"));
  ...
  TCHAR g_makeptis_path[MAX_PATH];
  TCHAR *p=g_makeptis_path;
  GetModuleFileName(g_hInstance,g_makeptis_path,sizeof(g_makeptis_path));
  while (*p) p++;
  while (p >= g_makeptis_path && *p != _T('\\')) p--;
  _tcscpy(p+1,_T("makeptis.exe"));

  WIN32_FIND_DATA fd;
  HANDLE h=FindFirstFile(g_makeptis_path,&fd);
  if (h==INVALID_HANDLE_VALUE)
  {
    if ((p-g_makeptis_path>4)&&(_totlower(*(p-1))==_T('n'))&&(_totlower(*(p-2))==_T('i'))&&(_totlower(*(p-3))==_T('b'))&&(*(p-4)==_T('\\')))
    {
      p -= 4;
      _tcscpy(p+1,_T("makeptis.exe"));
      h=FindFirstFile(g_makeptis_path,&fd);
      if (h==INVALID_HANDLE_VALUE)
      {
        MessageBox(hwndDlg,_T("Error finding makeptis.exe."),g_errcaption,MB_OK|MB_ICONSTOP);
        PostMessage(g_hwnd,WM_NOTIFYENDCOMPILE,0,0);
        return;
      }
    }
  ...
  wsprintf(g_cmdline,_T("\"%s\" %s \"%s\""),g_makeptis_path,g_options,nsifilename);
  DWORD id;
...
{
  static int ids[]={IDC_INFO,IDC_PTISICON,IDC_SZIPFRAME,IDC_BROWSE,IDC_ZIPFILE,IDC_ZIPINFO_SUMMARY,IDC_ZIPINFO_FILES,IDC_OFRAME,IDC_INAMEST,
