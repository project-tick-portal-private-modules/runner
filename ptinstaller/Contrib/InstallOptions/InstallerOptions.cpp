...
// LBS_NOTIFY              0x00000001 // LISTBOX/CHECKBOX/RADIOBUTTON/BUTTON/LINK - Notify PTIS script when control is "activated" (exact meaning depends on the type of control)
...
bool WINAPI SaveSettings(void) {
  ...
  // Tell PTIS which control was activated, if any
...
INT_PTR CALLBACK cfgDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg)
  {
    ...
    case WM_CTLCOLORLISTBOX:
      // let the PTIS window handle colors, it knows best
...
#undef DEFAULT_STYLES
    ...
    // PTIS window for the conversion, instead of this one.
...
void WINAPI showCfgDlg()
{
  ...
  // Tell PTIS to remove old inner dialog and pass handle of the new inner dialog
