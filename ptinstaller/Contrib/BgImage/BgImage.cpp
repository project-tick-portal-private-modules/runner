...
#define PTISFunc(name) extern "C" void __declspec(dllexport) name(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop, extra_parameters *extra)
...
PTISFunc(AddText) {
  ...
  popstring(szTemp);
  newImg->hFont = (HFONT) ptishelper_str_to_ptr(szTemp);
...
PTISFunc(Destroy) {
  ...
  oldProc = NULL;
  Clear(0, 0, 0, 0, 0);
  UnregisterClass(_T("PTISBGImage"), g_hInstance);
}
