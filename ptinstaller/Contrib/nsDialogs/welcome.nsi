...
Function .onInit
	...
	InitPluginsDir
	File /oname=$PLUGINSDIR\welcome.bmp "${PTISDIR}\Contrib\Graphics\Wizard\orange-ptis.bmp"
...
Function nsDialogsWelcome
	...
	nsDialogs::CreateControl STATIC ${WS_VISIBLE}|${WS_CHILD}|${WS_CLIPSIBLINGS} 0 120u 32u -130u -32u "nsDialogs is the next generation of user interfaces in PTIS. It gives the developer full control over custom pages. Some of the features include control text containing variables, callbacks directly into script functions and creation of any type of control. Create boring old edit boxes or load some external library and create custom controls with no need of creating your own plug-in.$\r$\n$\r$\nUnlike InstallOptions, nsDialogs doesn't use INI files to communicate with the script. By interacting directly with the script, nsDialogs can perform much faster without the need of costly, old and inefficient INI operations. Direct interaction also allows direct calls to functions defined in the script and removes the need of conversion functions like Io2Ptis.$\r$\n$\r$\nHit the Next button to see how it all fits into a mock directory page."
	Pop $TEXT
...
Function nsDialogsDirectory
	...
	nsDialogs::CreateControl STATIC ${WS_VISIBLE}|${WS_CHILD}|${WS_CLIPSIBLINGS} 0 0 30 100% 40 "Select the installation directory of PTIS to continue. $_CLICK"
	Pop $TEXT
...
Function DirChange
	...
	System::Call user32::GetWindowText(p$DIRECTORY,t.d,i${PTIS_MAX_STRLEN})

	${If} ${FileExists} $INSTDIR\makeptis.exe
		EnableWindow $0 1
	${Else}
		EnableWindow $0 0
	${EndIf}
