...
_BUNDLED_ZLIB_LIB = None

def AddZLib(env, platform, alias='install-utils'):
    """
    Uses bundled zlib from the repo when available.
    Falls back to platform zlib only if the bundled source is missing.
    """
    import os
    global _BUNDLED_ZLIB_LIB

    zlib_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'zlib'))
    if os.path.isdir(zlib_dir):
        if _BUNDLED_ZLIB_LIB is None:
            zlib_sources = [
                os.path.join(zlib_dir, name) for name in [
                    'adler32.c',
                    'compress.c',
                    'crc32.c',
                    'deflate.c',
            ...
            zlib_env = env.Clone()
            zlib_env.Append(CPPPATH = [zlib_dir])
            zlib_target = os.path.join('$BUILD_PREFIX', 'zlib', 'zlib')
            _BUNDLED_ZLIB_LIB = zlib_env.StaticLibrary(zlib_target, zlib_sources)

        env.Append(CPPPATH = [zlib_dir])
        env.Append(LIBS = [_BUNDLED_ZLIB_LIB])
        return
