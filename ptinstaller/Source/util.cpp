...
int update_bitmap(CResourceEditor* re, WORD id, const TCHAR* filename, int width/*=0*/, int height/*=0*/, int maxbpp/*=0*/) {
  ...
  if (IsBMPFile(hdr, size, &info) && 0 == fseek(f, 0, SEEK_SET) && LoadImageCanLoadFileFromResource(hdr, size))
  {
    if ((width && width != (int) info.Width) || (height && height != (int) info.Height))
      ...
      retval = -4;
    else if (re->UpdateResource(RT_BITMAP, id, PTIS_DEFAULT_LANG, f, CResourceEditor::TM_AUTO))
      retval = 0;
  }
  fclose(f);
  return retval;
...
char *CharNextExA(WORD codepage, const char *s, int flags) {
  ...
  const char* orglocct = PTISRT_setlocale_wincp(LC_CTYPE, codepage), *np;
...
#define setlocale_OEM(cat) PTISRT_setlocale_wincp((cat), 1252)
...
#include <langinfo.h>
#endif
bool PTISRT_Initialize() // Init function for POSIX
{
  iconvdescriptor id;
  ...
  return ptis_iconv_get_host_endian_ucs4_code() && IsValidCodePage(PTISRT_GetASCIICodepage());
}

const char* PTISRT_setlocale_wincp(int cat, unsigned int cp)
{
...
wchar_t* PTISRT_mbtowc(const char *Str)
{
  ...
  if ((size_t)-1 != cch && (d = (wchar_t*) malloc(++cch * sizeof(wchar_t))))
  {
    ...
    if ((size_t) -1 == cch) { PTISRT_free(d); d = 0; }
  }
  if (!errno) errno = ENOMEM;
  setlocale(LC_CTYPE, orglocct);
  return d;
}
char* PTISRT_wctomb(const wchar_t *Str)
{
  const char *orglocct = setlocale(LC_CTYPE, "");
  char *d = 0;
  const wchar_t *s = Str;
  ...
  if ((size_t) -1 != cb && (d = (char*) malloc(++cb)))
  {
    ...
    if ((size_t) -1 == cb) { PTISRT_free(d); d = 0; }
  }
  if (!errno) errno = ENOMEM;
  setlocale(LC_CTYPE, orglocct);
  return d;
}
char* PTISRT_wctombpath(const wchar_t *Path)
{
  char *p = PTISRT_wctomb(Path);
  if (p) PathConvertWinToPosix(p);
  return p;
}
char* PTISRT_ttombpath(const TCHAR *Path)
{
#ifdef _UNICODE
  return PTISRT_wctombpath(Path);
#else
  char *p = _tcsdup(Path);
...
{
  const char *path = PTISRT_wctomb(Path); // Caller should have converted to POSIX path
  if (!path) return -1;
...
#ifdef _UNICODE
static int PTISRT_wsystem(const wchar_t *wcmd)
{
...
const char* ptis_iconv_get_host_endian_ucs4_code()
{
...
bool ptis_iconv_reallociconv(iconv_t CD, char**In, size_t*cbInLeft, char**Mem, size_t&cbConverted)
{
  char *in, *heap = *Mem;
  UINT cbMem = 512;
  ...
  for(;;)
  {
    ...
    if (!p) break;
    heap = p, icvret = ptis_iconv_adaptor(iconv, CD, &in, &inleft, &out, &outleft);
...
{
  if (!g_nrt_iconv_narrowloc) PTISRT_Initialize(); // For winchar.cpp unit test
  switch(code_page)
  {
...
int WideCharToMultiByte(UINT CodePage, DWORD dwFlags, const wchar_t* lpWideCharStr,
  ...
  static char buffer[4096]; // BUGBUG: Should this be 4*PTIS_MAX_STRLEN for large string build?
  ...
  if (ptis_iconv_adaptor(iconv, cd, &in, &inbytes, &out, &outbytes) == (size_t) -1) {
    iconv_close(cd);
    return 0;
  }
...
int MultiByteToWideChar(UINT CodePage, DWORD dwFlags, LPCSTR lpMultiByteStr,
  ...
  static wchar_t buffer[4096]; // BUGBUG: Should this be 4*PTIS_MAX_STRLEN for large string build?
...
#else
  char *nativepath = PTISRT_ttombpath(pathname);
  if (!nativepath) return -1;
  int result = open(nativepath, flags);
  PTISRT_free(nativepath);
#endif
  return result;
}
#endif //! _WIN32
...
#else
  char *nativepath = PTISRT_wctombpath(path);
  if (nativepath)
  {
...
TCHAR* create_tempfile_path()
{
  ...
  if (cch && cch < COUNTOF(buftmpdir) && GetTempFileName(buftmpdir, _T("nst"), 0, buf))
    tfpath = _tcsdup(buf);
#else //! _WIN32
  char narrowpath[] = ("/tmp/makeptisXXXXXX");
  ...
  {
#ifdef _UNICODE
    assert(PTISRT_free_is_STDC_free());
    tfpath = PTISRT_mbtowc(narrowpath);
#else
...
#else // !_WIN32
  tstring result;
  char *rpret = 0, *inputpath = PTISRT_ttombpath(path.c_str());
...
switchcp:   cp = orgwinconoutcp, mbtwcf = 0, utf8 = false;
        ...
        i += 1+cbTrail;
        if (0xfeff == wchbuf[0] && 1 == cch) cch = 0; // MakePtisW is not a fan of the BOM, eat it.
        if (!cch) continue;
        wbuf[cchwb++] = wchbuf[0];
...
int sane_system(const TCHAR *command)
{
  ...
  //   `program files\ptis\makeptis.exe" "args`
...
#endif //~ _WIN32
#if defined(_WIN32) && defined(_UNICODE) && defined(MAKEPTIS)
...
int WINAPI WinStdIO_vfwprintf(FILE*strm, const wchar_t*Fmt, va_list val)
{
  ...
  {
    ExpandoString<wchar_t, PTIS_MAX_STRLEN> buf;
    errno = ENOMEM;
...
static HANDLE PTISRT_FastGetConsoleScreenHandle()
{
  extern WINSIO_OSDATA g_osdata_stdout, g_osdata_stderr;
  return WinStdIO_IsConsole(g_osdata_stdout) ? g_osdata_stdout.hNative : g_osdata_stderr.hNative;
}
bool PTISRT_Initialize() // Init function for MakePTIS Win32
{
...
#elif defined(_WIN32)
#define PTISRT_FastGetConsoleScreenHandle PTISRT_GetConsoleScreenHandle
bool PTISRT_Initialize() { return true; } // Init function for non-MakePTIS Win32 (PTISRT_DEFINEGLOBALS sets g_output and g_errout)
static HANDLE PTISRT_GetConsoleScreenHandle()
{
  DWORD cm;
...
{
#ifdef _WIN32
  HANDLE hWin32Con = PTISRT_FastGetConsoleScreenHandle();
...
void *operator new(size_t size) PTIS_CXX_THROWSPEC(bad_alloc)
{
  void *p = malloc(size);
...
void operator delete(void *p) PTIS_CXX_NOEXCEPT() { if (p) free(p); }
void operator delete [](void *p) PTIS_CXX_NOEXCEPT() { if (p) free(p); }
