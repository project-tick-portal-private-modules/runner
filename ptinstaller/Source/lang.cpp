...
// Default English strings. Should match PTIS_DEFAULT_LANG
...
int CEXEBuild::SetLangString(const TCHAR *name, LANGID lang, const TCHAR *str, BOOL LicenseData) {
  ...
  if (!LicenseData && _tcsclen(str) > PTIS_MAX_STRLEN-1)
    warning_fl(DW_LANGSTRING_OVERLONGLENGTH, _T("LangString \"%") NPRIs _T("\" longer than PTIS_MAX_STRLEN!"), name);
...
#define ADD_FONT(id) { \
          ...
          if (dlg) { \
            ...
            DWORD dwSize; \
            dlg = td.Save(dwSize); \
            res_editor->UpdateResource(RT_DIALOG, id+cur_offset, PTIS_DEFAULT_LANG, dlg, dwSize); \
            td.FreeSavedTemplate(dlg); \
          } \
        }
      ...
      if (!str || !*str) {
        ...
        if (us && *us) {
          ...
          if (!dstr)
            continue;
          if (i == NLF_BRANDING) {
            TCHAR temp[PTIS_MAX_STRLEN + sizeof(PTIS_VERSION)], temp2[COUNTOF(temp)];
            CreatePlatformStrfmt(dstr, temp2); // Change %s to %ls if required
            _stprintf(temp, temp2, PTIS_VERSION);
...
LanguageTable * CEXEBuild::LoadLangFile(TCHAR *filename) {
  ...
  // Check header
  TCHAR buf[PTIS_MAX_STRLEN];
  if (!GetNextNLFLine(lr, buf, PTIS_MAX_STRLEN, errlr)) {
l_readerr:
  ...
  if (nlf_version != NLF_VERSION) {
    if (nlf_version != 2 && nlf_version != 3 && nlf_version != 4 && nlf_version != 5) {
      ERROR_MSG(_T("Error: Language file version doesn't match PTIS version.\n"));
      return 0;
    }
  }

  // Get language ID
  if (!GetNextNLFLine(lr, buf, PTIS_MAX_STRLEN, errlr)) goto l_readerr;
  ...
  nlf->m_uCodePage = PTISRT_GetASCIICodepage();
  if (!GetNextNLFLine(lr, buf, PTIS_MAX_STRLEN, errlr)) goto l_readerr;
  TrimTrailingNewlines(buf);
  if (buf[0] != _T('-') || buf[1] != 0) {
  ...
  for (int i = 0; i < NLF_STRINGS_NO_SPECIAL; i++) {
    ...
    errlr = GetNextNLFLine(lr, buf, PTIS_MAX_STRLEN);
    if (_tcslen(buf) == PTIS_MAX_STRLEN-1) {
