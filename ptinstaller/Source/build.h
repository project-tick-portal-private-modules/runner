...
namespace MakeptisAPI {
  ...
  enum QUERYHOST_e {
    ...
    QH_SUPPORTEDVERSION, // [0x03006000] The host must return new highest makeptis compiler version it supports
  };
  typedef struct {
...
class DiagState {
  template<class C, class K, class V> void insert_or_assign(C&c, const K&k, V val)
  {
    ...
    std::pair<PTIS_CXX_TYPENAME C::iterator, bool> ret = c.insert(item);
...
class CEXEBuild {
  public:
    ...
    void initialize(const TCHAR *makeptis_path);
    ~CEXEBuild();
    ...
    HWND notify_hwnd;
    void notify(MakeptisAPI::datatransfer_e code, const TCHAR *data) const;
    #ifdef _WIN32
    ...
    #endif
    bool hostapi_request_data(MakeptisAPI::datatransfer_e operation, UINT minver, HOSTAPIREQUESTDATAPROC proc, void*cookie, const void* input, size_t inputsize) const;
  ...
  // Added by Sunil Kamath 11 June 2003
    ...
    /**
      ...
      * or Unicode for Unicode version of PTIS.  The language name is looked
