...
#define PTISRT_DEFINEGLOBALS() int g_display_errors=1; FILE *g_output=stdout, *g_errout=stderr
...
bool PTISRT_Initialize();
#define PTISRT_GetASCIICodepage() ( 1252 )
#ifndef _WIN32
// iconv const incoptistency workaround by Alexandre Oliva
template <typename T>
inline size_t ptis_iconv_adaptor
...
public:
  ...
  UINT Convert(void*inbuf, size_t*pInLeft, void*outbuf, size_t outLeft)
  {
    ...
    size_t ret = ptis_iconv_adaptor(iconv, *this, &in, pInLeft, &out, &outLeft);
    if ((size_t)(-1) == ret) 
      ret = 0, *pInLeft = 1;
    else
  ...
  static const char* GetHostEndianUCS4Code() { return ptis_iconv_get_host_endian_ucs4_code(); }
};
...
#endif
#define PTISRT_free_is_STDC_free() 1 // PTISRT_free == free
#define PTISRT_free(p) ( free((void*)(p)) )
