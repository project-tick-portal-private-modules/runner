...
#if defined(PTIS_CONFIG_COMPRESSION_SUPPORT) && defined(PTIS_COMPRESS_WHOLE)
HANDLE dbd_hFile=INVALID_HANDLE_VALUE;
static int dbd_size, dbd_pos, dbd_srcpos, dbd_fulllen;
#endif//PTIS_COMPRESS_WHOLE
...
static int PTISCALL calc_percent()
{
...
#if defined(PTIS_CONFIG_CRC_SUPPORT) || defined(PTIS_COMPRESS_WHOLE)
...
#endif//PTIS_CONFIG_CRC_SUPPORT || PTIS_COMPRESS_WHOLE
...
{C_ASSERT(opentotalwait == opentrywait * maxopentries);}
  ...
  GetModuleFileName(NULL, state_exe_path, PTIS_MAX_STRLEN);
retry:
...
#if defined(PTIS_CONFIG_CRC_SUPPORT) || defined(PTIS_CONFIG_SILENT_SUPPORT)
        cl_flags |= h.flags;
#endif
...
#else //!PTIS_CONFIG_CRC_ANAL
        left -= 4;
#endif//PTIS_CONFIG_CRC_ANAL
        // this is in case the end part is < 512 bytes.
        if (l > (DWORD)left) l=(DWORD)left;

#else//!PTIS_CONFIG_CRC_SUPPORT
...
#if !defined(PTIS_COMPRESS_WHOLE) || !defined(PTIS_CONFIG_COMPRESSION_SUPPORT)
...
#if (PTIS_MAX_EXEDATASIZE+0) > 0x7fffffffUL
...
static int PTISCALL __ensuredata(int amount)
{
