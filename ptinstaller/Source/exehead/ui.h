...
extern int PTISCALL ui_doinstall(void);
void PTISCALL update_status_text(int strtab, const TCHAR *text2);

extern int ui_dlg_visible;
extern HWND m_curwnd;

#ifdef PTIS_CONFIG_LOG
void PTISCALL build_g_logfile(void);
#endif

// sent to the last child window to tell it that the install thread is done
#define WM_NOTIFY_INSTPROC_DONE (WM_USER+0x4)

// sent to every child window to tell it it can start executing PTIS code
