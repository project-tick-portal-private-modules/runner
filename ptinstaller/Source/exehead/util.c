...
// This will produce a special section called ".ndata" (stands for ptis data)
// this way makeptis during build time, can search for this section by name
...
// ptis then removes the "DISCARDABLE" style from section (for safe)
#ifdef _MSC_VER
#  pragma bss_seg(PTIS_VARS_SECTION)
PTIS_STRING g_usrvarssection[1];
#  pragma bss_seg()
#  pragma comment(linker, "/section:" PTIS_VARS_SECTION ",rwd")
#else
#  ifdef __GNUC__
// GCC does not treat g_usrvarssection as a bss section so we keep the size as small as possible.
// PTIS_STRING g_usrvarssection[31] is required to remove this hack but that really bloats the exehead.
TCHAR g_usrvarssection[1] __attribute__((section (PTIS_VARS_SECTION)));
const PTIS_STRING*const g_usrvarsstart = (const PTIS_STRING*const) g_usrvarssection;
#  else
...
BOOL PTISCALL UserIsAdminGrpMember()
{
...
int PTISCALL my_GetDialogItemText(UINT idx, TCHAR *val)
{
  ...
  return GetDlgItemText(m_curwnd, idx, val, PTIS_MAX_STRLEN);
//  return my_GetWindowText(GetDlgItem(m_curwnd, idx), val, PTIS_MAX_STRLEN);
}
...
BOOL PTISCALL delete_with_ro_attr_handling(LPCTSTR fileordir,int flags) 
{
...
{
  static TCHAR lbuf[PTIS_MAX_STRLEN];
...
/*char PTISCALL lastchar(const char *str)
{
...
{
  static TCHAR tmp[PTIS_MAX_STRLEN];
  TCHAR *root;
...
char * PTISCALL mystrstriA(char *a, const char *b)
{
  int l = lstrlenA(b);
  while (lstrlenA(a) >= l)
  {
...
#ifndef _PTIS_NODEFLIB_CRTMEMCPY
...
void RenameViaWininit(const TCHAR* prevName, const TCHAR* newName)
{
  ...
  GetPTISString(wininit, g_header->str_wininit);
...
HKEY PTISCALL GetRegKeyAndSAM(HKEY hKey, REGSAM*pRS)
{
  ...
  *pRS = sam & ~(PTIS_REGSAM_PRIVATEMASK); // Filter away internal flags
...
{
  HKEY hKey;
  DWORD cb = PTIS_MAX_STRLEN*sizeof(TCHAR), rt, ec;
  ...
  if ((ec = RegKeyOpen(root, sub, KEY_READ|samview, &hKey)) == ERROR_SUCCESS)
  {
    ...
    RegCloseKey(hKey);
    out[PTIS_MAX_STRLEN-1] = 0; // Make sure the string is terminated. This could potentially truncate a long string by 1 character!
  }
...
{
  return lstrcpyn(out, in, PTIS_MAX_STRLEN);
}
...
{
  StrWideToACP(in, out, PTIS_MAX_STRLEN);
}
#endif
...
TCHAR ps_tmpbuf[PTIS_MAX_STRLEN*2];
...
TCHAR * PTISCALL GetPTISString(TCHAR *outbuf, int strtab)
{
  ...
  TCHAR *in = (TCHAR*)GetPTISStringNP(GetPTISTab(strtab));
  TCHAR *out = ps_tmpbuf;
  ...
  while (*in && out - ps_tmpbuf < PTIS_MAX_STRLEN)
  {
    ...
    if (nVarIdx < NS_SKIP_CODE)
    {
      ...
      if (nVarIdx == NS_SHELL_CODE)
      {
        ...
        */
        ...
        if (fldrs[1] & 0x80)
        {
          myRegGetStr(HKEY_LOCAL_MACHINE, SYSREGKEY, GetPTISStringNP(fldrs[1] & 0x3F), out, fldrs[1] & 0x40);
          if (!*out)
            GetPTISString(out, fldrs[3]);
          x = 0;
        }
        else if (fldrs[1] == CSIDL_SYSTEM) // Does not work on 95, 98 nor NT4. Works on ME and 2000+.
        {
          GetSystemDirectory(out, PTIS_MAX_STRLEN);
          x = 0;
        }
        else if (fldrs[1] == CSIDL_WINDOWS) // Does not work on 95, 98 nor NT4. Works on ME and 2000+.
        {
          GetWindowsDirectory(out, PTIS_MAX_STRLEN);
          x = 0;
        }
...
void PTISCALL validate_filename(TCHAR *in) {
  TCHAR *nono = _T("*?|<>/\":");
  TCHAR *out;
  TCHAR *out_save;
...
#endif//!PTIS_CONFIG_LOG_ODS && !PTIS_CONFIG_LOG_STDOUT
...
#endif//PTIS_CONFIG_LOG_TIMESTAMP

void log_printf(TCHAR *format, ...)
{
#if defined(PTIS_CONFIG_LOG_STDOUT)
  HANDLE hStdOut;
#endif
  va_list val;
  va_start(val,format);
  ...
  va_end(val);
#if defined(PTIS_CONFIG_LOG_ODS)
  if (log_dolog)
    OutputDebugString(log_text);
#elif defined(PTIS_CONFIG_LOG_STDOUT)
