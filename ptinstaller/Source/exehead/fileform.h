...
//   * ptis blocks (described in header->blocks)
//     pages (struct page)
...
// ptis strings
typedef TCHAR PTIS_STRING[PTIS_MAX_STRLEN];
...
#ifdef PTIS_CONFIG_COMPONENTPAGE
  int install_types[PTIS_MAX_INST_TYPES+1];
#endif
...
typedef struct
{
  ...
  int size_kb;
  TCHAR name[PTIS_MAX_STRLEN]; // '' for invisible sections
} section;
...
#if defined(_WIN64) && !defined(MAKEPTIS)
...
#define PTIS_INSTDIR_INVALID 1
#define PTIS_INSTDIR_NOT_ENOUGH_SPACE 2

#define FIELDN(x, y) (((int *)&x)[y])

#pragma pack(pop)

#define PTIS_MAX_EXEDATASIZE 0x7fffffffUL // Maximum size of .exe including compressed installer data.
#ifndef PTIS_CONFIG_CRC_ANAL
#define PTIS_MAX_EXEFILESIZE 0xffffffffUL // Maximum size of .exe including compressed installer data AND 3rd-party appended data. (Windows refuses to run .EXE files larger than 4 GiB)
#else
#define PTIS_MAX_EXEFILESIZE PTIS_MAX_EXEDATASIZE
#endif

#ifdef EXEHEAD
// the following are only used/implemented in exehead, not makeptis.

#if PTIS_MAX_EXEDATASIZE <= 0xffffffffUL
...
int PTISCALL isheader(firstheader *h); // returns 0 on not header, length_of_datablock on success

// returns nonzero on error
// returns 0 on success
// on success, m_header will be set to a pointer that should eventually be GlobalFree()'d.
// (or m_uninstheader)
const TCHAR * PTISCALL loadHeaders(int cl_flags);

int PTISCALL _dodecomp(int offset, HANDLE hFileOut, unsigned char *outbuf, int outbuflen);
...
BOOL PTISCALL ReadSelfFile(LPVOID lpBuffer, DWORD nNumberOfBytesToRead);
DWORD PTISCALL SetSelfFilePointer(LONG lDistanceToMove);
