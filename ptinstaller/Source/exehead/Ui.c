...
#if PTIS_MAX_INST_TYPES > 32 || PTIS_MAX_INST_TYPES < 1
#error invalid value for PTIS_MAX_INST_TYPES
#endif

int progress_bar_pos, progress_bar_len;

#if PTIS_MAX_STRLEN < 1024
static TCHAR g_tmp[4096];
#else
static TCHAR g_tmp[PTIS_MAX_STRLEN * 4];
#endif

static int m_page=-1,m_retcode,m_delta;
static page *g_this_page;

static void PTISCALL outernotify(int delta) {
...
static BOOL PTISCALL SetDlgItemTextFromLang_(HWND dlg, int id, int lid) {
  return my_SetDialogItemText(dlg,id+1000,GetPTISStringTT(lid));
}

static void PTISCALL SetNextDef()
{
  SendMessage(g_exec_flags.abort ? m_hwndCancel : m_hwndOK, BM_SETSTYLE, BS_DEFPUSHBUTTON, TRUE);
}

static void PTISCALL EnableNext(BOOL e)
{
  EnableWindow(m_hwndOK, e);
}

static void PTISCALL SetActiveCtl(HWND hCtl)
{
  SendMessage(g_hwnd, WM_NEXTDLGCTL, (WPARAM) hCtl, TRUE);
}

static BOOL PTISCALL LaunchURL(HWND hOwner, LPCTSTR URL, int ShowMode)
{
...
static void PTISCALL NotifyCurWnd(UINT uNotifyCode)
{
  if (m_curwnd)
...
#define IsPTISCtlColor(p) ( ( ((p)->lbStyle) <= 1 ) /* BS_SOLID||BS_HOLLOW */ \
...
static INT_PTR PTISCALL _HandleStaticBkColor(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
static TCHAR* update_caption()
{
  ...
  GetPTISString(gcap, LANG_CAPTION);
...
static void PTISCALL set_language()
{
...
lang_again:
  ...
  // reload section names
  {
    ...
    while (x--)
    {
      if (sec->name_ptr)
      {
        GetPTISString(sec->name, sec->name_ptr);
      }
      sec++;
    }
  }
...
FORCE_INLINE int PTISCALL ui_doinstall(void)
{
  ...
  g_exec_flags.plugin_api_version=PTISPIAPIVER_CURR;
#endif
  ...
  if (!is_valid_instpath(state_install_directory))
  {
    if (header->install_reg_key_ptr)
    {
      myRegGetStr(
        ...
        GetPTISStringNP(header->install_reg_key_ptr),
        GetPTISStringNP(header->install_reg_value_ptr),
        ps_tmpbuf,
        0
      );
      if (ps_tmpbuf[0])
  ...
  if (!is_valid_instpath(state_install_directory))
  {
    GetPTISString(state_install_directory,header->install_directory_ptr);
  }
...
#if defined(PTIS_SUPPORT_CODECALLBACKS) && defined(PTIS_CONFIG_ENHANCEDUI_SUPPORT)
...
nextPage:
    ...
    else
    {
      ...
      GetPTISString(state_click_next, this_page->clicknext);
      ...
      GetPTISString(g_tmp+mystrlen(g_tmp),this_page->caption);
      ...
      if (this_page->dlg_id > 0) // PTIS page
      {
...
#define LicIgnoreWMCommand g_cbLicRead // g_cbLicRead is only used in WM_INITDIALOG during EM_STREAMIN
  ...
  if (uMsg == WM_INITDIALOG)
  {
    TCHAR *l = (TCHAR *)GetPTISStringNP(GetPTISTab(this_page->parms[1]));
    int lt = *l;
    EDITSTREAM es = {
      (DWORD_PTR)(++l),
      0,
...
#ifndef _PTIS_NO_INT64_SHR
#define NRT_U64Shr32(v,s) ( (v) >> (s) )
#else
#define NRT_U64Shr32 Int64ShrlMod32
#endif

static void PTISCALL SetSizeText64(int dlgItem, int prefix, ULARGE_INTEGER kb64)
{
  ...
  wsprintf(
    GetPTISString(g_tmp, prefix) + mystrlen(g_tmp),
#else
  GetPTISString(g_tmp, prefix);
  wsprintf(
    g_tmp + mystrlen(g_tmp),
#endif
    _T("%u.%u%s%s"),
    intgr, fract,
    GetPTISString(scalestr, scale),
    GetPTISString(byte, LANG_BYTE)
    );

  my_SetDialogItemText(m_curwnd,dlgItem,g_tmp);
}
static void PTISCALL SetSizeText(int dlgItem, int prefix, unsigned kb)
{
  ULARGE_INTEGER kb64;
  kb64.QuadPart = kb;
...
static int PTISCALL _sumsecsfield(int idx)
{
  int total = 0;
  int x = num_sections;
...
static INT_PTR CALLBACK DirProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_COMMAND)
  {
    ...
    if (id == IDC_BROWSE)
    {
      static TCHAR bt[PTIS_MAX_STRLEN];
      BROWSEINFO bi = {0,};
      LPITEMIDLIST idlist;
      ...
      bi.lpszTitle = GetPTISString(bt, browse_text);
      ...
      if (idlist)
      {
          ...
          dir == state_install_directory) // only append to $INSTDIR (bug #1174184)
        {
          ...
          GetPTISStringTT(g_header->install_directory_auto_append);
  ...
  {
    static TCHAR s[PTIS_MAX_STRLEN];
    int error = 0;
    ...
    if (available_set)
      if (available.QuadPart < total)
        error = PTIS_INSTDIR_NOT_ENOUGH_SPACE;
...
static void FORCE_INLINE PTISCALL RefreshComponents(HWND hwTree, HTREEITEM *items)
{
  TVITEM item;
  int i, flags, state;
  section *sec;
...
int PTISCALL TreeGetSelectedSection(HWND tree, BOOL mouse)
{
...
#ifdef PTIS_CONFIG_COMPONENTPAGE_ALTERNATIVE
...
void PTISCALL ExecuteCallbackFunctionWithr0Int(int num,int r0)
{
...
#endif//PTIS_SUPPORT_CODECALLBACKS && PTIS_CONFIG_ENHANCEDUI_SUPPORT
...
static INT_PTR CALLBACK SelProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    for (i = 0; i < PTIS_MAX_INST_TYPES+1; i++)
    {
      if (install_types[i])
      {
        LRESULT j;
        if (i != PTIS_MAX_INST_TYPES) noCombo = 0;
        j=SendMessage(hwndCombo1,CB_ADDSTRING,0,(LPARAM)GetPTISStringTT(install_types[i]));
...
#if defined(PTIS_SUPPORT_CODECALLBACKS) && defined(PTIS_CONFIG_COMPONENTPAGE)
    if (wParam != 0)
    {
...
#endif//PTIS_SUPPORT_CODECALLBACKS && PTIS_CONFIG_COMPONENTPAGE
  ...
  if (uMsg == WM_NOTIFY_INSTTYPE_CHANGED)
  {
    ...
    {
      int c = (lParam == PTIS_MAX_INST_TYPES ? 1 : 0) << 3;// SW_SHOWNA=8, SW_HIDE=0
...
void PTISCALL update_status_text(int strtab, const TCHAR *text) {
  static TCHAR tmp[PTIS_MAX_STRLEN*2];
  LVITEM new_item;
...
static INT_PTR CALLBACK InstProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    log_printf3(_T("New install of \"%s\" to \"%s\""),GetPTISStringTT(LANG_NAME),state_install_directory);
  ...
  if (uMsg == WM_CONTEXTMENU && wParam == (WPARAM) linsthwnd)
  {
    ...
    if (count > 0)
    {
      ...
      POINT pt;
      AppendMenu(menu,MF_STRING,1,GetPTISStringTT(LANG_COPYDETAILS));
