...
#if !defined(PTIS_CONFIG_VISIBLE_SUPPORT) && !defined(PTIS_CONFIG_SILENT_SUPPORT)
#error One of PTIS_CONFIG_SILENT_SUPPORT or PTIS_CONFIG_VISIBLE_SUPPORT must be defined.
#endif
#ifdef PTIS_COMPRESS_WHOLE
extern HANDLE dbd_hFile;
#endif

TCHAR g_caption[PTIS_MAX_STRLEN*2]; // Why does this have to be PTIS_MAX_STRLEN*2?
...
PTIS_ENTRYPOINT_GUINOCRT
EXTERN_C void PTISWinMainNOCRT()
{
  int ret = 0;
...
#ifndef PTIS_SUPPORT_GETDLLVERSION
        "VERSION\0"
#endif
      ;
      const char *dll;
...
#ifndef _WIN64
  {
    ...
    enum { os_wow6432 = 30 };
    if (fp && ((BOOL(WINAPI*)(UINT))fp)(os_wow6432)) g_osinfo.WVProd |= PTIS_OSINFO_PROD_WOW64FLAG;
  }
#endif
...
#define END_OF_ARG(c) (c == _T(' ') || c == _T('\0'))
  ...
  GetTempPath(PTIS_MAX_STRLEN, state_temp_dir);
  if (!ValidateTempDir())
  {
    GetWindowsDirectory(state_temp_dir, PTIS_MAX_STRLEN - 5); // leave space for \Temp
    ...
    if (!ValidateTempDir())
    {
      ...
      GetTempPath(PTIS_MAX_STRLEN - 4, state_temp_dir); // leave space for \Low
...
retry_un_dir:
        ...
        GetPTISString(unexe, g_header->str_uninstchild); // '$TEMP\Un.exe'
        if (admin)
        {
...
#endif
            GetPTISString(unexecmd, g_header->str_uninstcmd); // '"$TEMP\Un.exe" $0 _?=$INSTDIR\'
