...
#if defined(PTIS_SUPPORT_ACTIVEXREG) || defined(PTIS_SUPPORT_CREATESHORTCUT)
HRESULT g_hres;
#endif

static int PTISCALL ExecuteEntry(entry *entry_);
 ...
 */
int PTISCALL resolveaddr(int v)
{
  if (v < 0)
  {
...
int PTISCALL ExecuteCodeSegment(int pos, HWND hwndProgress)
{
  while (pos >= 0)
  {
    int rv;
...
int PTISCALL ExecuteCallbackFunction(int num)
{
...
static TCHAR g_bufs[5][PTIS_MAX_STRLEN];
static int *g_parms;

void PTISCALL update_status_text_buf1(int strtab)
{
  update_status_text(strtab, g_bufs[1]);
}

#ifdef _WIN64
static INT_PTR PTISCALL GetIntPtrFromParm(int id)
{
  return strtoiptr(GetPTISStringTT(g_parms[id]));
}
#else
...
{
  LARGE_INTEGER v;
  const TCHAR *p = GetPTISStringTT(g_parms[id]);
...
static TCHAR * PTISCALL GetStringFromParm(int id_)
{
  int id = id_ < 0 ? -id_ : id_;
  TCHAR *result = GetPTISString(g_bufs[id >> 4], g_parms[id & 0xF]);
  if (id_ < 0) validate_filename(result);
  return result;
}

#ifdef PTIS_SUPPORT_REGISTRYFUNCTIONS
static HKEY PTISCALL GetRegRootKey(int RootKey)
{
...
static HKEY PTISCALL RegOpenScriptKey(REGSAM RS)
{
  HKEY hKey;
  return RegKeyOpen(GetRegRootKey(g_parms[1]), GetStringFromParm(0x22), RS|KEY_FROMSCRIPT, &hKey) ? NULL : hKey;
}
static HKEY PTISCALL RegCreateScriptKey(int RootKey, LPCTSTR SubKey, REGSAM RS)
{
  HKEY hKey;
...
static LONG PTISCALL DeleteRegTree(HKEY hThisKey, LPCTSTR SubKey, REGSAM SamviewAndFlags)
{
  HKEY hKey;
...
static LONG PTISCALL RegDeleteScriptKey(int RootKey, LPCTSTR SubKey, REGSAM SamviewAndFlags)
{
  HKEY hKey;
...
#pragma GCC diagnostic pop
  ...
  switch (which)
  {
    ...
    case EW_GETFULLPATHNAME:
      {
        ...
        TCHAR *p=var1;
        TCHAR *buf0=GetStringFromParm(0x00);
        if (!GetFullPathName(buf0,PTIS_MAX_STRLEN,p,&fp))
        {
          exec_error++;
          *p=0;
        }
        ...
        if (!parm2) GetShortPathName(p,p,PTIS_MAX_STRLEN);
      }
    break;
    case EW_SEARCHPATH:
      {
        ...
        if (!SearchPath(NULL,buf0,NULL,PTIS_MAX_STRLEN,p,&fp))
        {
          exec_error++;
          p[0]=0;
        }
    ...
    case EW_EXTRACTFILE:
      ...
      _tryagain:
        ...
        if (hOut == INVALID_HANDLE_VALUE)
        {
          ...
          GetPTISString(buf1,parm5); // use $0
        ...
        if (ret < 0)
        {
          if (ret == -2)
          {
            GetPTISString(buf0,LANG_ERRORWRITING);
            mystrcat(buf0,buf3);
          }
          else
          {
            GetPTISString(buf0,LANG_ERRORDECOMPRESSING);
          }
        ...
        if (newlen)
        {
          ...
          if (start>=0)
          {
            ...
            if (newlen)
            {
              ...
              if (newlen < PTIS_MAX_STRLEN) p[newlen]=0;
            }
          }
        }
      }
    ...
    case EW_READENVSTR:
      {
        TCHAR *p=var0;
        TCHAR *buf0=GetStringFromParm(0x01);
        if (!ExpandEnvironmentStrings(buf0,p,PTIS_MAX_STRLEN)
            || (parm2 && !lstrcmp(buf0, p)))
        {
          exec_error++;
          *p=0;
        }
        p[PTIS_MAX_STRLEN-1]=0;
      }
    break;
    ...
    case EW_PUSHPOP:
      {
        ...
        if (cnt) //Exch contributed by Fritz Elfert
        {
          ...
          if (!s)
          {
            ...
            my_MessageBox(GetPTISStringTT(LANG_INSTCORRUPTED),MB_OK|MB_ICONSTOP|(IDOK<<21));
            return EXEC_ERROR;
          }
    ...
    case EW_CREATEFONT:
    {
      ...
      GetPTISString(f.lfFaceName,parm1);
    ...
    case EW_REGISTERDLL:
      {
        ...
        if (SUCCEEDED(g_hres))
        {
          ...
          if (h)
          {
            // Jim Park: Need to use our special PTISGetProcAddress to convert
            // buf0 to char before calling GetProcAddress() which only takes 
            // chars.
            FARPROC funke = PTISGetProcAddress(h,buf0);
            if (funke)
            {
              exec_error--;
              if (parm2)
...
#else
            WCHAR *wsz = (LPWSTR) buf2; // buf2 + buf3 = WCHAR wsz[PTIS_MAX_STRLEN]
            hres = E_FAIL;
            if (MultiByteToWideChar(CP_ACP,0,buf1,-1,wsz,PTIS_MAX_STRLEN))
    ...
    case EW_READINISTR:
      {
        ...
        GetPrivateProfileString(buf0,buf1,errstr,p,PTIS_MAX_STRLEN-1,buf2);
    ...
    case EW_WRITEREG: // write registry value
      {
        ...
        if ((hKey = RegCreateScriptKey(rootkey, buf1, KEY_SET_VALUE)))
        {
          ...
          if (type == REG_BINARY)
          {
            ...
            size = GetCompressedDataFromDataBlockToMemory(parm3, data, (3 * PTIS_MAX_STRLEN)*sizeof(TCHAR));
    ...
    case EW_READREGSTR: // read registry string
      {
        ...
        if (hKey)
        {
          DWORD l = PTIS_MAX_STRLEN*sizeof(TCHAR), t;
          ...
          else
          {
            ...
            {
              exec_error += parm4;
              p[PTIS_MAX_STRLEN-1]=0; // RegQueryValueEx adds a null terminator, UNLESS the value is PTIS_MAX_STRLEN long
            }
          }
          RegCloseKey(hKey);
        }
...
#ifdef _UNICODE
    case EW_FPUTWS:
      // Jim Park/Wizou: in Unicode version of PTIS, EW_FPUTS still deals with ANSI files (conversion is done). We add EW_FPUTWS to deal with Unicode files.
#endif
      {
        ...
        else if (which==EW_FPUTS)
        {
          ...
          StrWideToACP(buf2, (LPSTR) buf1, PTIS_MAX_STRLEN);
...
#ifdef _UNICODE
    case EW_FGETWS:
      // Jim Park/Wizou: in Unicode version of PTIS, EW_FGETS still deals with ANSI files (conversion is done). We add EW_FGETWS to deal with Unicode files.
#endif
      {
        TCHAR *textout=var1;
        ...
        if (maxlen<1) break;
        if (maxlen > PTIS_MAX_STRLEN-1) maxlen=PTIS_MAX_STRLEN-1;
        if (*hptr)
        {
          TCHAR lc=0;
...
#if !defined(PTIS_CONFIG_LOG_ODS) && !defined(PTIS_CONFIG_LOG_STDOUT)
        if (parm1)
          build_g_logfile();
        else
          log_write(1);
    ...
    case EW_INSTTYPESET:
    {
      ...
      if ((unsigned int)x < (unsigned int)PTIS_MAX_INST_TYPES)
      {
        ...
        else // install type text
        {
          ...
          {
            GetPTISString(var1,g_header->install_types[x]);
          }
        }
      }
      else exec_error++;
