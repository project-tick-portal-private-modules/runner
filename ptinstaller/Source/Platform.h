...
#if defined(__cplusplus) && defined(MAKEPTIS) && (!defined(_MSC_VER) || _MSC_VER > 1200)
template<class T> class PTISCHARTYPE{ T _c; public: PTISCHARTYPE(){} PTISCHARTYPE(T c):_c(c){} operator T()const{ return _c; } };
typedef PTISCHARTYPE<unsigned short> WINWCHAR; // WINWCHAR is always UTF16LE and should not be passed to wcs* functions
#else
...
#  define PTIS_ENTRYPOINT_SIMPLEGUI \
...
#  ifdef __cplusplus
#    define PTIS_ENTRYPOINT_GUINOCRT \
       EXTERN_C void PTISWinMainNOCRT(); \
       int WINAPI WinMain(HINSTANCE hI,HINSTANCE hOld,char*cl,int sc) \
       {PTISWinMainNOCRT();return 0;}
#  endif
#endif
#ifndef PTIS_ENTRYPOINT_TMAIN
#  define PTIS_ENTRYPOINT_TMAIN
#endif
#ifndef PTIS_ENTRYPOINT_SIMPLEGUI // _tWinMain with valid hInstance, calls ExitProcess
#  define PTIS_ENTRYPOINT_SIMPLEGUI
#endif
#ifndef PTIS_ENTRYPOINT_GUINOCRT
#  define PTIS_ENTRYPOINT_GUINOCRT
#endif


#if defined(__clang__) && defined(__cplusplus) && __cplusplus < 201103L
#  define PTIS_CXX_THROWSPEC(throwspec) throw(throwspec) // Use exception specifications to avoid operator new missing-exception-spec warning
#else
#  define PTIS_CXX_THROWSPEC(ignoredthrowspec) // Ignore c++ exception specifications
#endif
#if defined(__cplusplus) && __cplusplus >= 201103L
#  define PTIS_CXX_NOEXCEPT() noexcept(true)
#else
#  define PTIS_CXX_NOEXCEPT() throw() // Can't specialize __declspec(nothrow) because MSVC requires it before the function name
#endif
#if defined(_MSC_VER) && _MSC_VER <= 1200
#  define PTIS_CXX_TYPENAME // VC6 can't handle typename in some places but GCC requires it
#else
#  define PTIS_CXX_TYPENAME typename
#endif
#define BUGBUG64TRUNCATE(cast,xpr) ( (cast) (xpr) )

/*
_tprintf on Windows/MSVCRT treats %s as TCHAR* and on POSIX %s is always char*!
Always use our NPRI* (PtisPRInt*[Narrow|Wide]) defines in format strings when calling 
