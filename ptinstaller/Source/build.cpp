...
namespace MakeptisAPI {
#ifdef _WIN64
  const TCHAR* SigintEventNameFmt = _T("makeptis win32 sigint event %Iu"); // %u is the notify HWND, this is to make sure we abort the correct instance
#else
  const TCHAR* SigintEventNameFmt = _T("makeptis win32 sigint event %u");
#endif
  const TCHAR* SigintEventNameLegacy = _T("makeptis win32 signint event"); // "sigNint" typo is part of the API now and cannot be changed
}
  ...
  m_exehead_size(0)
{
  ...
  definedlist.add(_T("PTIS_VERSION"), PTIS_VERSION);
  definedlist.add(_T("PTIS_PACKEDVERSION"), PTIS_PACKEDVERSION);
...
#include <ptis-defines.h>

  // no longer optional
  definedlist.add(_T("PTIS_SUPPORT_STANDARD_PREDEFINES"));
  definedlist.add(_T("PTIS_SUPPORT_NAMED_USERVARS"));
  definedlist.add(_T("PTIS_SUPPORT_LANG_IN_STRINGS"));

#ifdef _WIN32
  definedlist.add(_T("PTIS_WIN32_MAKEPTIS"));
#endif
#ifdef _UNICODE
  definedlist.add(_T("PTIS_UNICODE_MAKEPTIS")); // This define might go away once makeptis.exe is always unicode
#endif
  if (sizeof(void*) > 4) definedlist.add(_T("PTIS_MAKEPTIS64"));
  ...
  last_used_lang=PTIS_DEFAULT_LANG;
...
void CEXEBuild::initialize(const TCHAR *makeptis_path)
{
  tstring ptis_dir;
  const TCHAR *dir = _tgetenv(_T("PTISDIR"));
  if (dir) ptis_dir = dir;
  else {
#ifndef PTIS_CONFIG_CONST_DATA_PATH
    ptis_dir = get_dir_name(get_executable_dir(makeptis_path));
#else
    ptis_dir = _T(PREFIX_DATA);
#endif
  }
  definedlist.add(_T("PTISDIR"), ptis_dir.c_str());

  tstring includes_dir = ptis_dir;
...
char* convert_processed_string_to_ansi(char *out, const TCHAR *in, WORD codepage)
{
    ...
    for (;;)
    {
        ...
        if (NS_IS_CODE(i)) // Note: this includes '\0'
        {
            ...
            {
                WORD w = *p++; // special PTIS code is following by a WORD we need to output unchanged
                *out++ = LOBYTE(w);
                *out++ = HIBYTE(w);
            }
            in = p;
...
int CEXEBuild::add_page(int type)
{
...
#ifndef PTIS_CONFIG_LICENSEPAGE
  if (type == PAGE_LICENSE)
  {
    ERROR_MSG(_T("Error: can't add %") NPRIns _T(" page, %") NPRIns _T(" not defined.\n"), "license", "PTIS_CONFIG_LICENSEPAGE");
    return PS_ERROR;
  }
#endif
#ifndef PTIS_CONFIG_COMPONENTPAGE
  if (type == PAGE_COMPONENTS)
  {
    ERROR_MSG(_T("Error: can't add %") NPRIns _T(" page, %") NPRIns _T(" not defined.\n"), "components", "PTIS_CONFIG_COMPONENTPAGE");
    return PS_ERROR;
  }
#endif
  ...
  if (type == PAGE_COMPONENTS)
  {
    ERROR_MSG(_T("Error: can't add %") NPRIns _T(" page, %") NPRIns _T(" not defined.\n"), "uninstConfirm", "PTIS_CONFIG_UNINSTALL_SUPPORT");
    return PS_ERROR;
  }
#endif
...
#define REMOVE_ICON(id) if (disable_window_icon) { \
    BYTE* dlg = res_editor->GetResource(RT_DIALOG, id, PTIS_DEFAULT_LANG); \
    if (dlg) { \
      ...
      if (dt.RemoveItem(IDC_ULICON)) { \
        ...
        DWORD dwSize; \
        dlg = dt.Save(dwSize); \
        res_editor->UpdateResource(RT_DIALOG, id, PTIS_DEFAULT_LANG, dlg, dwSize); \
...
#ifdef PTIS_CONFIG_LICENSEPAGE
    if (!license_normal) {
      res_editor->UpdateResource(RT_DIALOG, IDD_LICENSE, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_LICENSE);
    if (!license_fsrb) {
      res_editor->UpdateResource(RT_DIALOG, IDD_LICENSE_FSRB, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_LICENSE_FSRB);
    if (!license_fscb) {
      res_editor->UpdateResource(RT_DIALOG, IDD_LICENSE_FSCB, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_LICENSE_FSCB);
#endif // PTIS_CONFIG_LICENSEPAGE
#ifdef PTIS_CONFIG_COMPONENTPAGE
    if (!selcom) {
      res_editor->UpdateResource(RT_DIALOG, IDD_SELCOM, PTIS_DEFAULT_LANG, 0, 0);
      res_editor->UpdateResource(RT_BITMAP, IDB_BITMAP1, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_SELCOM);
#endif // PTIS_CONFIG_COMPONENTPAGE
    if (!dir) {
      res_editor->UpdateResource(RT_DIALOG, IDD_DIR, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_DIR);
#ifdef PTIS_CONFIG_UNINSTALL_SUPPORT
    if (!uninstconfirm) {
      res_editor->UpdateResource(RT_DIALOG, IDD_UNINST, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_UNINST);
#endif // PTIS_CONFIG_UNINSTALL_SUPPORT
    if (!instlog) {
      res_editor->UpdateResource(RT_DIALOG, IDD_INSTFILES, PTIS_DEFAULT_LANG, 0, 0);
    }
    else REMOVE_ICON(IDD_INSTFILES);
    if (!main) {
      res_editor->UpdateResource(RT_DIALOG, IDD_INST, PTIS_DEFAULT_LANG, 0, 0);
      if (!build_compress_whole && !build_crcchk)
        res_editor->UpdateResource(RT_DIALOG, IDD_VERIFY, PTIS_DEFAULT_LANG, 0, 0);
    }
...
void CEXEBuild::PrepareInstTypes()
{
  if (!(cur_header->flags & CH_FLAGS_NO_CUSTOM))
    cur_header->install_types[PTIS_MAX_INST_TYPES] = DefineInnerLangString(NLF_COMP_CUSTOM);
...
#endif//PTIS_SUPPORT_MOVEONREBOOT
}
...
int CEXEBuild::SetVarsSection()
{
  try {
    ...
    int stringSize = PTIS_MAX_STRLEN * (build_unicode?2:1);
    if (!res_editor->SetPESectionVirtualSize(PTIS_VARS_SECTION, MaxUserVars * stringSize))
    {
      ERROR_MSG(_T("Internal compiler error #12346: invalid exehead cannot find section \"%") NPRIs _T("\"!\n"), _T(PTIS_VARS_SECTION));
      return PS_ERROR;
    }
  }
  catch (exception& err) {
...
int CEXEBuild::SetManifest()
{
  try {
    ...
    res_editor->UpdateResource(MAKEINTRESOURCE(24), 1, PTIS_DEFAULT_LANG, (LPBYTE) const_cast<char*>(manifest.c_str()), (DWORD) manifest.length());
  }
  catch (exception& err) {
...
retry_output:
  {
    ...
    notify(MakeptisAPI::NOTIFY_OUTPUT, full_path.c_str());
    INFO_MSG(_T("\nOutput: \"%") NPRIs _T("\"\n"), full_path.c_str());
...
int CEXEBuild::uninstall_generate()
{
  ...
  if (ubuild_entries.getlen() && uninstaller_writes_used)
  {
    ...
    if (start_offset)
    {
      ...
      if (udata_size > PTIS_MAX_EXEFILESIZE || add_db_data(&udata_in) < 0)
        return PS_ERROR;

      assert(PTIS_MAX_EXEFILESIZE <= ~(UINT32)0);
...
void CEXEBuild::warninghelper(DIAGCODE dc, bool fl, const TCHAR *fmt, va_list args)
{
  ...
  ExpandoString<TCHAR, COUNTOF(codbuf) + PTIS_MAX_STRLEN + 100> msgbuf;
  ...
  MakeptisAPI::datatransfer_e hostevent = MakeptisAPI::NOTIFY_WARNING;
  if (aserror)
    hostevent = MakeptisAPI::NOTIFY_ERROR, display_warnings = display_errors;
...
void CEXEBuild::ERROR_MSG(const TCHAR *s, ...) const
{
  ...
  {
    ExpandoString<TCHAR, PTIS_MAX_STRLEN + 100> buf;
    va_list val;
    va_start(val,s);
    buf.StrVFmt(s,val);
    va_end(val);

    notify(MakeptisAPI::NOTIFY_ERROR, buf.GetPtr());
    if (display_errors)
    {
...
void CEXEBuild::notify(MakeptisAPI::datatransfer_e code, const TCHAR *data) const
{
#ifdef _WIN32
  if (notify_hwnd)
  {
...
bool CEXEBuild::hostapi_request_data(MakeptisAPI::datatransfer_e operation, UINT minver, HOSTAPIREQUESTDATAPROC proc, void*cookie, const void* input, size_t inputsize) const
{
  using namespace MakeptisAPI;
#ifdef _WIN32
  struct helper {
  ...
  struct handler {
    static bool CALLBACK proc(void*cookie, HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
    {
      ...
      if (Msg == WM_COPYDATA && pCDS->cbData > sizeof(TCHAR) && pCDS->cbData <= io[2] * sizeof(TCHAR))
      {
        ...
        return (io[0] = (pCDS->dwData == MakeptisAPI::PROMPT_FILEPATH));
      }
      return false;
    }
  };
...
int CEXEBuild::initialize_default_plugins(bool newtargetarc)
{
  ...
  tstring searchPath = definedlist.find(_T("PTISDIR"));
...
int CEXEBuild::set_target_architecture_data()
{
  ...
  if (build_unicode)
  {
    definedlist.set(_T("PTIS_UNICODE"));
    definedlist.set(_T("PTIS_CHAR_SIZE"), _T("2"));
  }
  else
  {
    definedlist.del(_T("PTIS_UNICODE"));
    definedlist.set(_T("PTIS_CHAR_SIZE"), _T("1"));
  }
  definedlist.set(_T("PTIS_PTR_SIZE"), t64 ? _T("8") : _T("4"));

  const TCHAR* tsuff = get_target_suffix(m_target_type, _T(""));
  if (!*tsuff) return PS_ERROR;
  tstring cpu = get_string_prefix(tsuff, _T("-"));
  definedlist.set(_T("PTIS_CPU"), cpu.c_str()); // Used by Library.nsh to pick the correct RegTool

  struct { TARGETTYPE tt; const TCHAR *def; const TCHAR *val; } static const tdef[] = {
    { TARGET_X86ANSI,    _T("PTIS_IX86"),  _T("300") },
    { TARGET_X86UNICODE, _T("PTIS_IX86"),  _T("400") },
    { TARGET_AMD64,      _T("PTIS_AMD64"), _T("1")   },
    { TARGET_ARM64,      _T("PTIS_ARM64"), _T("1")   }
  };
