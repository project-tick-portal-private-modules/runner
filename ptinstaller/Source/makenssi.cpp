...
#define PTIS_COPYYEARS _T("1999-2025")

using namespace std;

PTISRT_DEFINEGLOBALS();
...
{
  if (!g_dopause) return;
  if (g_display_errors) _ftprintf(g_output,_T("MakePTIS done - hit enter to close..."));
  fflush(stdout);
  int a;
...
static UINT_PTR QueryHost(HWND hHost, UINT_PTR wp, UINT_PTR lp=0, UINT_PTR def=0) { return hHost ? SendMessage(hHost, MakeptisAPI::QUERYHOST, wp, lp) : def; }
#else //! _WIN32
...
{
  _ftprintf(g_output,_T("MakePTIS %") NPRIs _T(" - Copyright ") PTIS_COPYYEARS _T(" Contributors\n")
...
{
  _ftprintf(g_output,_T("Copyright (C) ") PTIS_COPYYEARS _T(" Nullsoft and Contributors\n\n")
       _T("This license applies to everything in the PTIS package, except where otherwise\n")
       _T("noted.\n\n")
...
static void print_usage()
{
  _ftprintf(g_output,_T("Usage:\n")
         _T("  ")         _T("makeptis [ option | script.nsi | - ] [...]\n")
         _T("\n")
         _T("Options:\n")
         _T("  ") OPT_STR _T("CMDHELP [item] prints out help for 'item', or lists all commands\n")
         _T("  ") OPT_STR _T("HDRINFO prints information about what options makeptis was compiled with\n")
         _T("  ") OPT_STR _T("LICENSE prints the makeptis software license\n")
         _T("  ") OPT_STR _T("VERSION prints the makeptis version and exits\n")
         ...
         _T("  ") OPT_STR _T("NOCONFIG disables inclusion of <path to makeptis.exe>") PLATFORM_PATH_SEPARATOR_STR _T("ptisconf.nsh\n")
...
#endif
         _T("You can use a double-dash to end options processing: makeptis -- -ins.nsi\n"));
  fflush(g_output);
}
...
#ifdef PTIS_HPUX_ALLOW_UNALIGNED_DATA_ACCESS
extern "C" void allow_unaligned_data_access();
#endif

static inline int makeptismain(int argc, TCHAR **argv)
{
  ...
  if (!PTISRT_Initialize())
  {
    _ftprintf(stderr,_T("PTISRT_Initialize failed!\n"));
    return 1;
  }
  ...
  if (oneoutputstream || !(1 & QueryHost(hostnotifyhandle,MakeptisAPI::QH_ENABLESTDERR,0,1)))
    g_errout=g_output;
  ...
  if (hostnotifyhandle)
  {
    ...
    LRESULT mr=QueryHost(hostnotifyhandle,MakeptisAPI::QH_OUTPUTCHARSET,lp);
  ...
  if (!argc)
  {
    _ftprintf(g_output,PTIS_VERSION);
    fflush(g_output);
    return 0;
  }
  ...
  while (argpos < argc)
  {
    ...
    else if (IS_OPT(argv[argpos]) && _tcscmp(argv[argpos], _T("-")) && !in_files)
    {
               ...
               argv[argpos][2] >= _T('0') && argv[argpos][2] <= _T('5') && !argv[argpos][3])
      {
        ...
        if (p == 5) build.warning(DW_CMDLINE_HIGHPRIORITY,_T("makeptis is running in REALTIME priority mode!"));
#else
    ...
    else
    {
      ...
      if (!noconfig)
      {
        noconfig=true;
        tstring main_conf;
        TCHAR* env_var = _tgetenv(_T("PTISCONFDIR"));
        if (env_var == NULL)
...

      {
        ...
        build.notify(MakeptisAPI::NOTIFY_SCRIPT,nsifile.c_str());
        TCHAR bufcpdisp[20];
...
#else
    const TCHAR *cmd = _tgetenv(_T("PTISLAUNCHCOMMAND"));
...
#endif
#endif
  int retval = makeptismain(argc,argv);
#ifndef NDEBUG
#ifdef _MSC_VER
...
int main(int argc, char **argv)
{
  ...
  if (wargv) 
  {
    for ( ; wargc < argc; ++wargc )
      if ((p = PTISRT_mbtowc(argv[wargc]))) wargv[wargc] = p; else break;
  }
  if (wargc == argc)
  ...
  if (wargv) for ( int i = 0; i < wargc; ++i ) PTISRT_free(wargv[i]);
  free(wargv);
#endif
  return errno;
}
