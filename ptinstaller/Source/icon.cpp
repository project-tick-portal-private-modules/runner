...
{
  return load_icon_res(re, RT_GROUP_ICON, id, PTIS_DEFAULT_LANG);
}
...
void set_main_icon(CResourceEditor* re, WORD wIconId, IconGroup icon1, IconGroup icon2)
{
  ...
  re->UpdateResource(RT_GROUP_ICON, wIconId, PTIS_DEFAULT_LANG, group1, (DWORD)group_size, CResourceEditor::TM_RAW); // Update the group entries but not the images
  destroy_icon_group(group1);

  // delete old icons
  unsigned i = 1;
  while (i <= MAIN_ICON_LAST_IMAGE && re->UpdateResource(RT_ICON, i++, PTIS_DEFAULT_LANG, 0, 0));
  ...
  for (order_index = 0; order_index < order.size(); order_index++)
  {
    ...
    re->UpdateResource(RT_ICON, size_index + 1, PTIS_DEFAULT_LANG, data, size);
...
int generate_unicons_offsets(LPBYTE exeHeader, size_t exeHeaderSize, LPBYTE uninstIconData, WORD wIconId)
{
  try
  {
    ...
    offset = re.GetResourceOffset(RT_GROUP_ICON, wIconId, PTIS_DEFAULT_LANG);
    ...
    *(LPDWORD) seeker = FIX_ENDIAN_INT32(offset);
    ...
    {
      offset = re.GetResourceOffset(RT_ICON, icon_index, PTIS_DEFAULT_LANG);
      ...
      DWORD real_size = re.GetResourceSize(RT_ICON, icon_index, PTIS_DEFAULT_LANG);
