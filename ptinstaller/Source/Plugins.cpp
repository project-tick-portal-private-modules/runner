...
// VC6 cannot handle PTISException(const tstring& msg) : std::runtime_error(string(TtoCString(msg))) {}
struct PTISExceptionInner : public std::runtime_error
{
  PTISExceptionInner(const char* msg) : std::runtime_error(string(msg)) {} // Unicode
  PTISExceptionInner(const string&msg) : std::runtime_error(msg) {} // Ansi
};
struct PTISException : public PTISExceptionInner
{
  PTISException(const tstring& msg) : PTISExceptionInner(TtoCString(msg)) {}
};
...
struct PrintPluginDirsHelper {
  ...
  {
    std::/*unordered_*/set<PTIS_CXX_TYPENAME STL::mapped_type<C>::type
#ifdef _WIN32
      , Plugins::strnocasecmp
#endif
    > seen;
    for (PTIS_CXX_TYPENAME C::const_iterator it = c.begin(); it != c.end(); ++it)
    {
