...
void TestQuaZipDir::entryList_data()
{
	...
	QTest::newRow("simple") << "simple.zip"
							...
							<< static_cast<int>(QDir::Unsorted) << (QStringList() << "test2.txt" << "subdir/") << -1;
	QTest::newRow("separate dir") << "sepdir.zip" << (QStringList() << "laj/" << "laj/lajfile.txt") << ""
	...
	QTest::newRow("separate dir (subdir listing)")
		<< "sepdirsub.zip" << (QStringList() << "laj/" << "laj/lajfile.txt") << "laj" << QStringList()
		<< static_cast<int>(QDir::NoFilter) << static_cast<int>(QDir::Unsorted) << (QStringList() << "lajfile.txt")
		<< -1;
	QTest::newRow("dirs only") << "dirsonly.zip" << (QStringList() << "file" << "dir/") << "" << QStringList()
							   << static_cast<int>(QDir::Dirs) << static_cast<int>(QDir::Unsorted)
							   << (QStringList() << "dir/") << -1;
	QTest::newRow("files only") << "filesonly.zip" << (QStringList() << "file1" << "parent/dir/" << "parent/file2")
	...
	QTest::newRow("sorted") << "sorted.zip"
							<< (QStringList() << "file1" << "parent/subdir/" << "parent/subdir2/file3" << "parent/file2"
											  << "parent/file0")
							<< "parent" << QStringList() << static_cast<int>(QDir::NoFilter)
							<< static_cast<int>(QDir::Name)
							<< (QStringList() << "file0" << "file2" << "subdir/" << "subdir2/") << -1;
	QTest::newRow("sorted dirs first") << "sorted-dirs.zip"
									   << (QStringList() << "file1" << "parent/subdir/" << "parent/subdir2/file3"
														 << "parent/file2" << "parent/file0")
									   << "parent" << QStringList() << static_cast<int>(QDir::NoFilter)
									   << static_cast<int>(QDir::Name | QDir::DirsFirst)
									   << (QStringList() << "subdir/" << "subdir2/" << "file0" << "file2") << -1;
	QTest::newRow("sorted dirs first reversed")
		...
		<< (QStringList() << "subdir2/" << "subdir/" << "file2" << "file0") << -1;
	QTest::newRow("sorted by size") << "sorted-size.zip" << (QStringList() << "file000" << "file10") << "/"
									<< QStringList() << static_cast<int>(QDir::NoFilter) << static_cast<int>(QDir::Size)
									<< (QStringList() << "file10" << "file000") << -1;
	QTest::newRow("sorted by time") << "sorted-time.zip"
									<< (QStringList() << "file04" << "file03" << "file02" << "subdir/subfile") << "/"
									<< QStringList() << static_cast<int>(QDir::NoFilter) << static_cast<int>(QDir::Time)
									<< (QStringList() << "subdir/" << "file04" << "file02" << "file03") << -1;
	QTest::newRow("sorted by type") << "sorted-type.zip" << (QStringList() << "file1.txt" << "file2.dat") << "/"
									<< QStringList() << static_cast<int>(QDir::NoFilter) << static_cast<int>(QDir::Type)
									<< (QStringList() << "file2.dat" << "file1.txt") << -1;
	QTest::newRow("name filter") << "name-filter.zip" << (QStringList() << "file01" << "file02" << "laj") << "/"
								 << QStringList("file*") << static_cast<int>(QDir::NoFilter)
								 << static_cast<int>(QDir::Name) << (QStringList() << "file01" << "file02") << -1;
	QTest::newRow("case sensitive") << "case-sensitive.zip" << (QStringList() << "a" << "B") << "/" << QStringList()
									<< static_cast<int>(QDir::NoFilter) << static_cast<int>(QDir::Name)
									<< (QStringList() << "B" << "a") << static_cast<int>(QuaZip::csSensitive);
	QTest::newRow("case insensitive") << "case-insensitive.zip" << (QStringList() << "B" << "a") << "/" << QStringList()
									  << static_cast<int>(QDir::NoFilter) << static_cast<int>(QDir::Name)
									  << (QStringList() << "a" << "B") << static_cast<int>(QuaZip::csInsensitive);
}
...
void TestQuaZipDir::entryList()
{
	...
	{
		dir.setCaseSensitivity(static_cast<QuaZip::CaseSensitivity>(caseSensitivity));
...
void TestQuaZipDir::filePath()
{
	...
	QuaZipDir dir(&zip);
	QVERIFY(dir.cd("dir"));
	QCOMPARE(dir.relativeFilePath("/root.txt"), QString::fromLatin1("../root.txt"));
