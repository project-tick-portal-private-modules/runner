...
void TestQuaZipFile::zipUnzip_data()
{
	...
	QTest::newRow("simple") << "simple.zip"
							...
							<< QByteArray() << QByteArray() << Z_DEFLATED << Z_DEFAULT_COMPRESSION << false << false
							<< -1;
#ifdef HAVE_BZIP2
	...
	QTest::newRow("Cyrillic") << "cyrillic.zip"
							  << (QStringList() << QString::fromUtf8("русское имя файла с пробелами.txt"))
							  << QByteArray("IBM866") << QByteArray() << Z_DEFLATED << Z_DEFAULT_COMPRESSION << false
							  << false << -1;
	QTest::newRow("Unicode") << "unicode.zip"
							 << (QStringList()
								 << QString::fromUtf8("Українське сало.txt") << QString::fromUtf8("Vin français.txt")
								 << QString::fromUtf8("日本の寿司.txt") << QString::fromUtf8("ქართული ხაჭაპური.txt"))
							 << QByteArray("") << QByteArray() << Z_DEFLATED << Z_DEFAULT_COMPRESSION << false << true
							 << -1;
	QTest::newRow("password") << "password.zip" << (QStringList() << "test.txt") << QByteArray()
							  << QByteArray("PassPass") << Z_DEFLATED << Z_DEFAULT_COMPRESSION << false << false << -1;
	QTest::newRow("zip64") << "zip64.zip" << (QStringList() << "test64.txt") << QByteArray() << QByteArray()
						   << Z_DEFLATED << Z_DEFAULT_COMPRESSION << true << false << -1;
	QTest::newRow("large enough to flush")
		<< "flush.zip" << (QStringList() << "flush.txt") << QByteArray() << QByteArray() << Z_DEFLATED
		<< Z_DEFAULT_COMPRESSION << true << false << 65536 * 2;
}
...
void TestQuaZipFile::zipUnzip()
{
	...
	foreach (QString fileName, fileNames)
	{
		...
		QVERIFY(archived.open(QIODevice::ReadOnly, password.isEmpty() ? NULL : password.constData()));
	...
	if (!password.isEmpty())
	{
		...
		QVERIFY(readError != ZIP_OK || archivedData.isEmpty() || archivedData != originalData); // corrupt data or
																								// empty, depends on the
																								// platform
	}
	testUnzip.close();
...
void TestQuaZipFile::zipUnzipLarge()
{
	...
	QuaZipFile outFile(&testZip);
	if (!outFile
			 .open(QIODevice::WriteOnly, QuaZipNewInfo(fileName, inFile.fileName()), NULL, 0, Z_DEFLATED, Z_BEST_SPEED))
	{ // we use best speed here so test is faster..
		qDebug("outFile.open() backend error, code %d", outFile.getZipError());
		QFAIL("outFile.open() returned FALSE");
	}
	for (qint64 pos = 0, len = inFile.size(); pos < len;)
...
void TestQuaZipFile::bytesAvailable_data()
{
	...
	QTest::newRow("simple") << "test.zip"
							<< (QStringList() << "test0.txt" << "testdir1/test1.txt"
											  << "testdir2/test2.txt" << "testdir2/subdir/test2sub.txt")
							<< -1;
	QTest::newRow("large enough to flush") << "flush.zip" << (QStringList() << "test.txt") << 65536 * 4;
}
...
void TestQuaZipFile::bytesAvailable()
{
	...
	foreach (QString fileName, fileNames)
	{
		...
		QCOMPARE(_zipFile.read(fileInfo.size() - 1).size(), static_cast<int>(fileInfo.size() - 1));
...
void TestQuaZipFile::getFileInfo()
{
	...
	QCOMPARE(info32.uncompressedSize, static_cast<quint32>(info64.uncompressedSize));
...
void TestQuaZipFile::setFileAttrs()
{
	...
	}
	{
		...
		QFile::Permissions usedPermissions = QFile::WriteOwner | QFile::ReadOwner | QFile::ExeOwner | QFile::WriteGroup
										   | QFile::ReadGroup | QFile::ExeGroup | QFile::WriteOther | QFile::ReadOther
										   | QFile::ExeOther;
		QCOMPARE(info.getPermissions() & usedPermissions, srcInfo.permissions() & usedPermissions);
...
void TestQuaZipFile::largeFile()
{
	...
	QCOMPARE(fakeLargeZip.getFileInfoList()[0].uncompressedSize, static_cast<quint32>(0));
	fakeLargeZip.close();
