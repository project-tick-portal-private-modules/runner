...
void TestQuaZipFileInfo::getNTFSTime()
{
	...
	QFileInfo fileInfo("tmp/test.txt");
	{
		...
		mTicks		 = (static_cast<qint64>(base.date().daysTo(lm.date())) * Q_UINT64_C(86400000)
					+ static_cast<qint64>(base.time().msecsTo(lm.time())))
			   * Q_UINT64_C(10000);
		aTicks = (static_cast<qint64>(base.date().daysTo(lr.date())) * Q_UINT64_C(86400000)
				  + static_cast<qint64>(base.time().msecsTo(lr.time())))
			   * Q_UINT64_C(10000);
		cTicks = (static_cast<qint64>(base.date().daysTo(cr.date())) * Q_UINT64_C(86400000)
...
void TestQuaZipFileInfo::getExtTime_data()
{
	...
	QTest::newRow("no times") << QString::fromUtf8("noTimes") << static_cast<quint8>(0) << static_cast<quint16>(1)
							  << QList<qint32>() << static_cast<quint16>(1) << QList<qint32>() << QDateTime()
							  << QDateTime() << QDateTime();
	QTest::newRow("all times") << QString::fromUtf8("allTimes") << static_cast<quint8>(7) << static_cast<quint16>(13)
							   << (QList<qint32>() << 1 << 2 << 3) << static_cast<quint16>(5) << (QList<qint32>() << 1)
	...
	QTest::newRow("no ac time") << QString::fromUtf8("noAcTime") << static_cast<quint8>(5) << static_cast<quint16>(9)
								<< (QList<qint32>() << 1 << 3) << static_cast<quint16>(5) << (QList<qint32>() << 1)
								<< QDateTime(QDate(1970, 1, 1), QTime(0, 0, 1), COMPAT_UTC_TZ) << QDateTime()
								<< QDateTime(QDate(1970, 1, 1), QTime(0, 0, 3), COMPAT_UTC_TZ);
	QTest::newRow("negativeTime") << QString::fromUtf8("negativeTime") << static_cast<quint8>(1)
								  << static_cast<quint16>(5) << (QList<qint32>() << -1) << static_cast<quint16>(5)
								  << (QList<qint32>() << -1)
								  << QDateTime(QDate(1969, 12, 31), QTime(23, 59, 59), COMPAT_UTC_TZ) << QDateTime()
								  << QDateTime();
}
...
void TestQuaZipFileInfo::parseExtraField_data()
{
	...
	QTest::newRow("empty") << QByteArray() << QuaExtraFieldHash();
	{
		QuaExtraFieldHash oneZeroIdWithNoData;
		oneZeroIdWithNoData[0] = QList<QByteArray>() << QByteArray();
		QTest::newRow("one zero ID with no data") << QByteArray("\x00\x00\x00\x00", 4) << oneZeroIdWithNoData;
	}
	{
		QuaExtraFieldHash oneZeroIdWithData;
		oneZeroIdWithData[0] = QList<QByteArray>() << QByteArray("DATA", 4);
		QTest::newRow("one zero ID with data") << QByteArray("\x00\x00\x04\x00"
															 "DATA",
															 8)
											   << oneZeroIdWithData;
	}
	{
		...
		QTest::newRow("one non zero ID with data") << QByteArray("\x01\x00\x04\x00"
																 "DATA",
																 8)
	...
	}
	{
		...
		QTest::newRow("two IDs") << QByteArray("\x00\x00\x05\x00"
											   "DATA0"
											   "\x01\x00\x05\x00"
											   "DATA1",
											   18)
	...
	}
	{
		...
		QTest::newRow("repeated ID") << QByteArray("\x00\x00\x05\x00"
												   "DATA0"
												   "\x00\x00\x05\x00"
												   "DATA1",
												   18)
	...
	}
	{
		...
		QTest::newRow("large ID") << QByteArray("\x02\x01\x04\x00"
												"DATA",
												8)
								  << largeID;
	}
	{
		...
		QTest::newRow("large ID") << (QByteArray("\x00\x00\xff\xff", 4) + QByteArray(65535, 'x')) << largeData;
	}
