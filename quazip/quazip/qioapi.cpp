...
{
	auto func = pfilefunc->zfile_func64.zopen64_file != nullptr ? pfilefunc->zfile_func64.zopen64_file
...
int ZCALLBACK qiodevice_seek_file_func(voidpf /*opaque UNUSED*/, voidpf stream, uLong offset, int origin)
{
	...
	switch (origin)
	{
		case ZLIB_FILEFUNC_SEEK_CUR: qiodevice_seek_result = (static_cast<QIODevice*>(stream))->pos() + offset; break;
		case ZLIB_FILEFUNC_SEEK_END: qiodevice_seek_result = (static_cast<QIODevice*>(stream))->size() - offset; break;
		case ZLIB_FILEFUNC_SEEK_SET: qiodevice_seek_result = offset; break;
		default: return -1;
	}
