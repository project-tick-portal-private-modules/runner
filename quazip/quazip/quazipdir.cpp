...
class QuaZipDirPrivate : public QSharedData
{
  ...
  private:
	QuaZipDirPrivate(QuaZip* _zip, const QString& _dir = QString()) : zip(_zip), dir(_dir)
	{}
	QuaZip* zip;
	QString dir;
...
QuaZipDir::QuaZipDir(QuaZip* zip, const QString& dir) : d(new QuaZipDirPrivate(zip, dir))
{
...
bool QuaZipDir::cd(const QString& directoryName)
{
	...
	if (dirName.contains(QLatin1String("/")))
	{
		...
		{
#ifdef QUAZIP_QUAZIPDIR_DEBUG
			qDebug("QuaZipDir::cd(%s): going to %s", dirName.toUtf8().constData(), step.toUtf8().constData());
#endif
			if (!dir.cd(step))
				return false;
		}
...
{
  public:
	inline QuaZipDirRestoreCurrent(QuaZip* _zip) : zip(_zip), currentFile(zip->getCurrentFileName())
	{}
...
int QuaZipDirComparator::compareStrings(const QString& string1, const QString& string2)
{
...
bool QuaZipDirComparator::operator()(const QuaZipFileInfo64& info1, const QuaZipFileInfo64& info2)
{
	QDir::SortFlags order = sort & (QDir::Name | QDir::Time | QDir::Size | QDir::Type);
	...
	switch (order)
	{
		case QDir::Name: result = compareStrings(info1.name, info2.name) < 0; break;
		case QDir::Type:
			extDiff = compareStrings(getExtension(info1.name), getExtension(info2.name));
			if (extDiff == 0)
			{
		...
		case QDir::Time:
			...
			break;
		default: qWarning("QuaZipDirComparator(): Invalid sort mode 0x%2X", static_cast<unsigned>(sort)); return false;
	}
									 ...
									 TFileInfoList& result) const
{
	...
	do
	{
		...
		if (!nmfltr.isEmpty() && !QDir::match(nmfltr, relativeName))
			continue;
		bool ok;
		QuaZipFileInfo64 info = QuaZipDir_getFileInfo(zip, &ok, relativeName, isReal);
		if (!ok)
		{
			return false;
		}
	...
	{
		qDebug("%s\t%s", info.name.toUtf8().constData(), info.dateTime.toString(Qt::ISODate).toUtf8().constData());
	}
#endif
...
QList<QuaZipFileInfo> QuaZipDir::entryInfoList(QDir::Filters filters, QDir::SortFlags sort) const
{
...
QList<QuaZipFileInfo64> QuaZipDir::entryInfoList64(QDir::Filters filters, QDir::SortFlags sort) const
{
	return entryInfoList64(QStringList(), filters, sort);
}

QStringList QuaZipDir::entryList(const QStringList& nameFilters, QDir::Filters filters, QDir::SortFlags sort) const
{
	QStringList result;
	if (!d->entryInfoList(nameFilters, filters, sort, result))
		return QStringList();
...
QStringList QuaZipDir::entryList(QDir::Filters filters, QDir::SortFlags sort) const
{
...
bool QuaZipDir::exists(const QString& filePath) const
{
...
#ifdef QUAZIP_QUAZIPDIR_DEBUG
	qDebug("QuaZipDir::exists(): looking for %s", fileName.toUtf8().constData());
	for (const auto& entry : entries)
	{
		qDebug("QuaZipDir::exists(): entry: %s", entry.toUtf8().constData());
	}
#endif
	...
	return entries.contains(fileName, cs) || entries.contains(fileName + QLatin1String("/"), cs);
}
