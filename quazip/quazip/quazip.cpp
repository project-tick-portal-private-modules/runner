...
void QuaZip::close()
{
	...
	switch (p->mode)
	{
		case mdNotOpen: return;
		case mdUnzip: p->zipError = unzClose(p->unzFile_f); break;
		case mdCreate:
		case mdAppend:
		case mdAdd:
			p->zipError = zipClose(p->zipFile_f,
								   p->comment.isNull() ? nullptr
								   : isUtf8Enabled()   ? p->comment.toUtf8().constData()
													   : p->commentCodec->fromUnicode(p->comment).constData());
			break;
		default: qWarning("QuaZip::close(): unknown mode: %d", static_cast<int>(p->mode)); return;
	}
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo* info) const
{
	...
	if (info == nullptr)
	{ // Very unlikely because of the overloads
		return false;
	}
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo64* info) const
{
	...
	if (!isOpen() || !hasCurrentFile())
		return false;
	if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(p->unzFile_f, &info_z, nullptr, 0, nullptr, 0, nullptr, 0))
		!= UNZ_OK)
		return false;
	fileName.resize(info_z.size_filename);
	extra.resize(info_z.size_file_extra);
	comment.resize(info_z.size_file_comment);
	if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(p->unzFile_f,
														 nullptr,
														 fileName.data(),
														 fileName.size(),
														 extra.data(),
	...
	info->name =
		(info->flags & UNZ_ENCODING_UTF8) ? QString::fromUtf8(fileName) : p->fileNameCodec->toUnicode(fileName);
	info->comment =
		(info->flags & UNZ_ENCODING_UTF8) ? QString::fromUtf8(comment) : p->commentCodec->toUnicode(comment);
	info->extra	   = extra;
	info->dateTime = QDateTime(QDate(info_z.tmu_date.tm_year, info_z.tmu_date.tm_mon + 1, info_z.tmu_date.tm_mday),
...
QString QuaZip::getCurrentFileName() const
{
	...
	QString result =
		(file_info.flag & UNZ_ENCODING_UTF8) ? QString::fromUtf8(fileName) : p->fileNameCodec->toUnicode(fileName);
	if (result.isEmpty())
		return result;
	// Add to directory map
