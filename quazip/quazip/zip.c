...
static int Write_LocalFileHeader(zip64_internal* zi,
								 const char* filename,
...
extern zipFile ZEXPORT zipOpen2_64(voidpf file,
								   int append,
								   zipcharpc* globalcomment,
						  ...
						  uLong version_to_extract)
{
	...
	if (err == ZIP_OK)
	{
		if (zi->ci.flag & ZIP_ENCODING_UTF8)
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)63, 2); /* Version 6.3 is required for
																						 Unicode support */
		else if (zi->ci.zip64)
	...
	{
		if (zi->ci.zip64)
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xFFFFFFFF, 4); /* compressed size,
																								 unknown */
		else
	...
	{
		if (zi->ci.zip64)
			err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xFFFFFFFF, 4); /* uncompressed size,
																								 unknown */
		else
										   ...
										   int zip64)
{
	...
	if (method == 0 && (level == 0 || (zi->flags & ZIP_WRITE_DATA_DESCRIPTOR) == 0)
	...
	if (password != NULL)
		zi->ci.flag |= 1;
	if (version_to_extract >= 20 && ((zi->flags & ZIP_WRITE_DATA_DESCRIPTOR) != 0 || (zi->flags & ZIP_SEQUENTIAL) != 0))
		zi->ci.flag |= 8;
	...
	if ((err == Z_OK) && (password != NULL))
	{
		...
		if (crcForCrypting == 0)
		{
			crcForCrypting = zi->ci.dosDate << 16; /* ATTANTION! Without this row, you don't unpack your password
													  protected archive in other app. */
		}
...
extern int ZEXPORT zipCloseFileInZipRaw64(zipFile file, ZPOS64_T uncompressed_size, uLong crc32)
{
	...
	if (compressed_size >= 0xffffffff || uncompressed_size >= 0xffffffff || zi->ci.pos_local_header >= 0xffffffff)
	{
		...
		zip64local_putValue_inmemory(zi->ci.central_header + 6,
									 (uLong)((zi->ci.flag & ZIP_ENCODING_UTF8) ? 63 : 45),
									 2);
	}
		...
		if ((zi->flags & ZIP_SEQUENTIAL) == 0)
		{
			...
			if (uncompressed_size >= 0xffffffff || compressed_size >= 0xffffffff)
			{
				if (zi->ci.pos_zip64extrainfo > 0)
				{
					...
					if (ZSEEK64(zi->z_filefunc, zi->filestream, zi->ci.pos_zip64extrainfo + 4, ZLIB_FILEFUNC_SEEK_SET)
						!= 0)
						err = ZIP_ERRNO;
...
int Write_Zip64EndOfCentralDirectoryRecord(zip64_internal* zi, uLong size_centraldir, ZPOS64_T centraldir_pos_inzip)
{
	...
	if (err == ZIP_OK) /* version needed */
		err = zip64local_putValue(&zi->z_filefunc,
								  zi->filestream,
								  (uLong)((zi->ci.flag & ZIP_ENCODING_UTF8) ? 63 : 45),
								  2);
	...
	{
		{
			if (zi->number_entry >= 0xFFFF)
				err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xffff, 2); /* use value in ZIP64
																								 record */
			else
