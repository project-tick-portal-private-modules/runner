					  ...
					  int strategy)
{
	...
	if ((mode & WriteOnly) && !(mode & ReadOnly))
	{
		...
		p->setZipError(zipOpenNewFileInZip4_64(
			p->zip->getZipFile(),
			p->zip->isUtf8Enabled() ? info.name.toUtf8().constData()
			...
			p->zip->isUtf8Enabled() ? info.comment.toUtf8().constData()
...
qint64 QuaZipFile::csize() const
{
	...
	p->setZipError(UNZ_OK);
	if (p->zip == nullptr || p->zip->getMode() != QuaZip::mdUnzip)
		return -1;
